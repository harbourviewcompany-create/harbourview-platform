...
export function DesktopDecisionIntelBridge({ signals }: { signals: DashboardSignal[] }) {
  ...
  return (
    <section className="mx-5 mt-4 rounded-2xl border border-[#c6a55a]/20 bg-[#07111f] p-4" aria-label="Decision intelligence dossiers">
      <div className="mb-3 flex items-center justify-between gap-4">
        ...
        <span className="text-[10px] text-white/45">Open the full evidence, unknowns and decision posture</span>
      </div>
      <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
        {signals.slice(0, 4).map(signal => {
          ...
          return (
            <Link
              key={signal.id}
              href={`/dashboard/intel/events/${encodeURIComponent(eventId)}?returnTo=${encodeURIComponent(returnTo)}`}
            ...
            >
              ...
              <span className="mt-2 block text-[10px] text-white/45">Open dossier →</span>
            </Link>
          )
        })}
      </div>
