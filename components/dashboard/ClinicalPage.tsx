/**
 * Harbourview — Clinical Page (full Command Centre section)
 * Wires Evidence Engine + Jurisdiction Briefing + Professional Pathway
 * + fixed empty states + clean hierarchy.
 *
 * Drop into components/dashboard/ClinicalPage.tsx and render when
 * page === 'clinical' inside CommandCentre.
 */
...
export interface ClinicalPageProps {
  ...
  flag?: string;
  roleLabel?: string;
  briefing: JurisdictionBriefing | null;
  pathway: ProfessionalPathway | null;
  initialEvidence: EvidenceRecord[];
  whatChanged: WhatChangedEvent[];
  attention: ClinicalAttentionItem[];
  nextActions: ClinicalNextAction[];
}
      ...
      {record.keyFindings.length > 0 && (
        <ul className="mt-1 space-y-1">
          {record.keyFindings.slice(0, 3).map((f, i) => (
            <li key={i} className={`text-xs leading-relaxed ${muted}`}>
              · {f}
            </li>
          ))}
        </ul>
...
export default function ClinicalPage({
  ...
  const [results, setResults] = useState<EvidenceRecord[]>(initialEvidence);
  ...
  const runSearch = (q: string, filter: string | null) => {
    startTransition(() => {
      ...
      const found = searchEvidence(q, { domain, jurisdiction: iso2 });
      setResults(found);
      setSearched(true);
    });
  };
  ...
  const showEmpty = searched ? evidenceCount === 0 : initialEvidence.length === 0 && !query;
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <section className={`${card} p-4`}>
        ...
        <PrimaryLink>Open clinician workspace →</PrimaryLink>
      </section>
      ...
      <section className={`${card} p-4`}>
        ...
        <div className="mt-5 space-y-3">
          {isPending && (
            <p className={`text-sm ${muted}`}>Searching reviewed evidence…</p>
          )}

          {!isPending && showEmpty && (
            <div className="rounded-lg bg-white/[0.03] px-3.5 py-4">
              <p className="text-sm font-medium text-white/80">
                {searched ? "No matching reviewed records" : "0 reviewed records for this context"}
              </p>
      ...
      {briefing && (
        <section className={`${card} p-4`}>
          <div className="flex items-center justify-between gap-2">
            <SectionLabel>
              Jurisdiction briefing · {briefing.country}
            </SectionLabel>
          ...
          {briefing.primaryAuthority.url && (
            <PrimaryLink href={briefing.primaryAuthority.url}>
              Open primary source ↗
            </PrimaryLink>
          )}
        </section>
      ...
      <section className={`${card} p-4`}>
        ...
        {nextActions.map((action) => (
          <div key={action.id} className="mt-2">
            ...
            <PrimaryLink href={action.primaryActionUrl}>
      ...
      {briefing && (
        <section className={`${card} p-4`}>
          ...
          <PrimaryLink>Jurisdiction command →</PrimaryLink>
        </section>
      )}
      ...
      {pathway && (
        <section className={`${card} p-4`}>
          ...
          <PrimaryLink>Clinical workspace →</PrimaryLink>
        </section>
      )}
    </div>
  );
