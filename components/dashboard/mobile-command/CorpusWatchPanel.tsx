...
export function CorpusWatchPanel({ countryLabel }: { countryLabel?: string }) {
  ...
  const scanLabel = scannedAt
    ? new Date(scannedAt).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
    : null
    ...
    <section aria-labelledby="hvm2-corpus-watch" className="hvm2-local-intel-groups">
      ...
      <div className="hvm2-metric-grid hvm2-regulatory-metrics">
        <Metric
          ...
          detail={registry?.ready ? 'Identity rows live' : 'Not confirmed'}
        />
          ...
          {hits.map(hit => (
            <article key={hit.signalId} className="hvm2-intel-record-card">
              <div className="hvm2-intel-meta-row">
                ...
                {hit.contextual ? <StatusPill tone="ok">Context</StatusPill> : null}
              </div>
      ...
      {state === 'ready' && hits.length === 0 ? (
        <EmptyState
          title={silent ? 'Watch silence in current window' : 'No corpus matches'}
          detail={
            [
              note || 'Active keyword rules did not hit the current public reviewed feed window.',
              scanLabel ? `Scanned ${scanLabel}.` : null,
              feedAgeHours != null ? `Newest feed item ~${Math.round(feedAgeHours)}h old.` : null,
            ].filter(Boolean).join(' ')
          }
        />
      ) : null}

      {state === 'ready' && scanLabel ? (
        <p className="hvm2-intel-unknown">Last corpus scan {scanLabel}</p>
      ) : null}
    </section>
  )
}
