...
export default function MobileCommandCentreRebuild(props: Props) {
  ...
  const sectionElements: Record<SectionId, ReactNode> = {
    ...
    clinical: <ClinicalSection sectionRef={model.sectionRef('clinical')} roleShort={model.roleShort} jurisdiction={model.countryLabel} programStatus={props.countryIntel?.briefing_program_status} medicalStatus={props.countryIntel?.medical_status} patientAccess={props.countryIntel?.briefing_patient_access} physicianAccess={props.countryIntel?.briefing_physician_access} commandHref={model.commandHref} />,
