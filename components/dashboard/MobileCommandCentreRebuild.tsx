  ...
  const sectionElements: Record<SectionId, ReactNode> = {
    ...
    'personal-briefing': (
      <>
        ...
        {/* Above is a deterministic quick-glance summary (computed local
            ...
            watch-rule-driven weekly market briefings + jurisdiction
            orientation briefings — which had no home anywhere (desktop or
            mobile) before this session. */}
    ...
    search: (
      <>
        ...
        {/* Above is explicitly session-scope only (its own copy says so and
            links out to /dashboard/signals/search for full-corpus search).
            Below is that full-corpus search, inline, so the redirect target
            actually delivers what the link promises. */}
        <div className="hvm2-section">
