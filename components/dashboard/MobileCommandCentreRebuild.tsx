...
export default function MobileCommandCentreRebuild(props: MobileCommandCentreProps) {
  ...
  const sectionElements: Record<SectionId, ReactNode> = {
    overview: <OverviewSection sectionRef={model.sectionRef('overview')} countryLabel={model.countryLabel} roleLabel={model.roleLabel} publicSummary={props.countryIntel?.public_summary} marketAccessStatus={props.countryIntel?.market_access_status} reviewStatus={model.reviewStatus} dataCompleteness={model.dataCompleteness} firstAction={model.nextActions[0]} onOpenActions={() => model.navigateToSection('next-actions')} />,
