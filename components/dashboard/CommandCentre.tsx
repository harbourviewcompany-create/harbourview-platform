      ...
      <div className="cc-app">
        ...
        <main className="cc-workspace">
          <section className="cc-panel cc-market">
            ...
            <div className="cc-view-bar"><div className="cc-views">{(['cannabis','equipment','consumables','new-products','services','opportunities','wanted'] as MarketView[]).map(v => <button key={v} className={`cc-view${view === v ? ' active' : ''}`} onClick={() => setView(v)}>{VIEW_TAB_LABELS[v]}</button>)}</div><button className="cc-customize-btn" onClick={() => setDrawerOpen(true)}>Customize</button></div><div className="cc-pipeline">{([
            ...
            <div className="cc-market-grid"><div className="cc-market-block"><div className="cc-block-title"><b>{VIEW_BLOCK_TITLES[view]}</b><span>{rows.length} rows · region-filtered</span></div><div className="cc-rows">{rows.length === 0 ? <div style={{padding:"28px 16px",textAlign:"center",color:"rgba(245,241,232,0.3)",fontSize:12,letterSpacing:"0.08em"}}>No listings yet for this category — submit via intake or check back soon.</div> : rows.map(r => <article key={r[2]} className="cc-row"><div className={`cc-spec ${r[0]}`} /><div><div className="cc-type">{r[1]}</div><h4>{r[2]}</h4><p>{r[3]}</p><TagPills str={r[4]} /><TrustBar str={r[5]} /></div><div className="cc-action-box"><strong>{r[7]}</strong><small>{r[1].toLowerCase()}</small><Link className="cc-row-action" href={VIEW_HREF[view]}>{r[6]}</Link><Link className="cc-secondary" href="/marketplace/wanted">Watch</Link></div></article>)}</div></div></div>
          </section>
