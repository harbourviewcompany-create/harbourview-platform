    ...
    <section className="rounded-3xl border border-amber-300/30 bg-amber-300/[0.08] p-4 text-amber-50 shadow-2xl shadow-black/20">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
        {['Request review', 'Submit evidence', 'Track this jurisdiction', 'View adjacent verified markets'].map((action) => (
          <Link key={action} href={dto.primaryAction.href} className="rounded-2xl border border-amber-200/20 bg-black/20 px-3 py-3 text-center font-semibold text-amber-100">{action}</Link>
        ))}
      </div>
    </section>
  )
...
export function CountryRoleHeader({ dto }: { dto: CountryRolePublicDto }) {
  return (
    <header className="rounded-[2rem] border border-[#d6b66b]/20 bg-[#071425]/95 p-5 shadow-2xl shadow-black/30">
      ...
      <h1 className="mt-4 font-serif text-3xl leading-tight tracking-[-0.04em] text-[#f7f1df] sm:text-5xl">{dto.role.shortLabel} command center</h1>
      <p className="mt-4 text-sm leading-7 text-white/68">{dto.role.operatingQuestion}</p>
      <div className="mt-5 flex flex-col gap-3 sm:flex-row">
        <Link href={dto.primaryAction.href} className="rounded-2xl bg-[#d6b66b] px-5 py-4 text-center text-sm font-bold text-[#08111f] shadow-xl shadow-[#d6b66b]/10">{dto.primaryAction.label}</Link>
        <Link href={`/country/${dto.country.countrySlug}/role/${dto.role.slug}#modules`} className="rounded-2xl border border-white/15 px-5 py-4 text-center text-sm font-semibold text-white/80">View modules</Link>
      </div>
    </header>
  )
}
...
export function RoleActionBar({ dto }: { dto: CountryRolePublicDto }) {
  return <section className="flex snap-x gap-3 overflow-x-auto pb-1">{dto.actions.map((action) => <Link className="min-w-max rounded-full border border-[#d6b66b]/20 bg-[#d6b66b]/10 px-4 py-3 text-xs font-semibold text-[#f3dda5]" key={action} href={dto.primaryAction.href}>{labelize(action)}</Link>)}</section>
}
...
export function RolePriorityStack({ dto }: { dto: CountryRolePublicDto }) {
  ...
  return <section id="modules" className="grid gap-3">{topModules.map((moduleKey, index) => <RoleModuleCard key={moduleKey} moduleKey={moduleKey} index={index} />)}</section>
}
...
export function RoleSelector({ dto }: { dto: CountryRolePublicDto }) {
  ...
  return <section className="rounded-3xl border border-white/10 bg-black/20 p-4"><p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-white/45">Switch role in this family</p><div className="mt-3 flex flex-wrap gap-2">{familyRoles.map((role) => <Link key={role.slug} href={`/country/${dto.country.countrySlug}/role/${role.slug}`} className={`rounded-full border px-3 py-2 text-xs ${role.slug === dto.role.slug ? 'border-[#d6b66b] bg-[#d6b66b]/15 text-[#f4dfaa]' : 'border-white/10 text-white/62'}`}>{role.shortLabel}</Link>)}</div></section>
}

export function MobileCommandStack({ dto }: { dto: CountryRolePublicDto }) {
  return <main className="mx-auto flex w-full max-w-5xl flex-col gap-4 px-4 py-4 sm:px-6 lg:py-8"><CountryRoleHeader dto={dto} /><EvidenceGapState dto={dto} /><RoleMissionCard dto={dto} /><RoleActionBar dto={dto} /><RolePriorityStack dto={dto} /><RoleSelector dto={dto} /></main>
}
