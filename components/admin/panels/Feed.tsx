...
type FeedRow = { id: string; status: string; workspace_id: string; published_at: string };

export function Feed({ api, toast }: { api: AdminApi; toast: (msg: ToastMsg) => void }) {
  const [rows, setRows] = useState<FeedRow[]>([]);
  const [loading, setLoading] = useState(true);
  const load = useCallback(async()=>{setLoading(true);try{setRows(await api.get("hv_public_feed","select=*&order=created_at.desc&limit=200"));}catch(e){toast({type:"error",text: e instanceof Error ? e.message : String(e)});}setLoading(false);},[]);
  useEffect(()=>{load();},[load]);
