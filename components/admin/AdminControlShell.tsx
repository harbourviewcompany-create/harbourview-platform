 ...
 * Replaces the dual-chrome problem (HubPanel SPA sidebar + separate admin layout pills).
 */
...
const shellCss = `
  ...
  .acs-content.acs-flush{padding:0;}
...
function ShellInner({ children }: { children: React.ReactNode }) {
  ...
  const isHubSpa = pathname === '/admin/hub' || pathname.startsWith('/admin/hub/')
  const title =
    CONTROL_SURFACE_NAV.flatMap((g) => g.items).find((i) => i.id === activeId)?.label ||
    'Admin'
...
export function AdminControlShell({ children }: { children: React.ReactNode }) {
  return (
    <Suspense
      fallback={
