...
export function MarketOverviewSheet({
  ...
  // In-component ISO2-keyed cache — avoids re-fetching the same country
  // within a session as the user clicks around the globe.
  ...
  useEffect(() => {
    ...
    async function load(): Promise<JurisdictionBriefing | null> {
      ...
      // Subnational: code contains a hyphen (e.g. 'CA-ON', 'AU-NSW', 'DE-BY')
      if (code.includes('-')) {
        const parentIso2 = code.split('-')[0]

        // Try subnational row first — query directly by state_iso2 so we don't
        // need a client-side slug map (covers AU, DE, all other subnational seeds)
        ...
        // Fall back to parent country row
