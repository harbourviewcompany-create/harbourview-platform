-- hv_intelligence_outcome_check() only recognized status='published' as a
-- valid delivered digest edition. Since 2026-08-31, most days have been
-- served by the 'published_manual' fallback (daily_digest_manual_fallback
-- migration) -- which this check didn't know about, so it has been raising
-- false digest_stale / digest_missed_today alerts every single day the
-- fallback ran, for the last ~2 weeks, despite a real edition going out
-- each time.
--
-- Now recognizes both statuses as satisfying "a digest was published", and
-- adds an informational (non-alerting) note when the most recent edition
-- was the manual fallback rather than LLM-curated, since that's still a
-- useful signal that LLM providers remain degraded.
...
begin
  ...
  where status in ('published', 'published_manual')
    and (
  ...
  select status = 'published_manual' into v_last_digest_manual
  from public.daily_digest
  where digest_date = v_last_digest;
  ...
  if v_digest_age_days is null or v_digest_age_days > 2 then
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object(
      ...
      'severity', 'critical',
      'message', format(
        'Daily Digest has no published edition (LLM or manual-pass) for %s days. Check run_daily_digest / LLM providers.',
  ...
  if v_last_digest_manual then
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object(
      'code', 'digest_on_manual_fallback',
      'severity', 'info',
      'message', format('Most recent edition (%s) was the non-LLM manual-pass fallback, not LLM-curated -- likely means all configured LLM providers are still degraded.', v_last_digest)
    ));
  end if;
  ...
  return jsonb_build_object(
    ...
    'metrics', jsonb_build_object(
      ...
      'last_digest_is_manual_fallback', coalesce(v_last_digest_manual, false),
