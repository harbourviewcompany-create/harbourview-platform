...
begin
  with candidates as (
    select s.id,
      left(
        regexp_replace(
          regexp_replace(
            regexp_replace(coalesce(nullif(btrim(s.summary),''), s.headline, 'Source text is limited.'), E'\\s+', ' ', 'g'),
            E'^(Home|Search|Menu|Skip to content)\\s+', '', 'i'
          ),
          E'(.{40,})\\s+\\1(\\s+\\1)+', '\\1', 'gi'
        ),
        700
      ) as cleaned_summary,
      left(btrim(s.headline),500) as clean_headline,
      s.cat, s.score, s.verification, s.source, s.date
  ...
  updated as (
    ...
    set analysis = jsonb_build_object(
      'what_changed',
        case
          when c.cleaned_summary is not null
               and length(c.cleaned_summary) >= 40
               and lower(c.cleaned_summary) not like lower(c.clean_headline) || '% ' || lower(c.clean_headline) || '%'
            then c.cleaned_summary
          else c.clean_headline
        end,
      'who_is_affected',
        case
          ...
          when lower(coalesce(c.cleaned_summary,'')) ~ 'investor|stock|share|market'
            then 'Investors and market participants exposed to the reported company, sector, or market development.'
          else 'Organizations and market participants directly exposed to the reported development.'
        end,
      'deadline', null,
      'recommended_action',
        case
          ...
          else 'Monitor the underlying source and verify the development before taking material operational action.'
        end,
      'confidence_rationale',
        case
          when nullif(btrim(c.verification),'') is not null
            then 'Rules-based analysis using the recorded signal text and metadata; source verification is marked "' || btrim(c.verification) || '".'
          else 'Rules-based analysis using the recorded signal text and metadata; source verification metadata is limited.'
        end,
      'analysis_quality',
        case
          when lower(coalesce(c.source,'')) ~ 'source engine'
               or lower(coalesce(c.cleaned_summary,'')) ~ 'home|search|menu|watchlist|featured|copyright'
            then 'limited_source_text'
          else 'rules_based'
        end
    ),
    analysis_generated_at = now(),
    analysis_backend = 'rules-v1'
    from candidates c
    where s.id = c.id
    returning 1
  )
