...
with base as (
  select c.id, c.iso_alpha2 as jurisdiction_key, c.country_name, c.country_slug, c.iso_alpha3,
         ...
         case when c.iso_alpha2 ~ '^[A-Z]{2}, c.medical_status, c.adult_use_status,
...
grant select on public.v_jurisdiction_data_depth to anon, authenticated; then 'national' else 'subnational' end as jurisdiction_level,
