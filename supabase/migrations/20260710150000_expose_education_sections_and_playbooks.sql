...
CREATE OR REPLACE VIEW api.education_module_sections AS SELECT * FROM public.education_module_sections;
GRANT SELECT ON api.education_module_sections TO anon, authenticated;

CREATE OR REPLACE VIEW api.jurisdiction_playbooks AS SELECT * FROM public.jurisdiction_playbooks;
...
CREATE VIEW api.public_signals AS SELECT * FROM regulatory_signals.public_signals;
