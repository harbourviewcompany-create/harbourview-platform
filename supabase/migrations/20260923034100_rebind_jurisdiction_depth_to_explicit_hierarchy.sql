-- Rebind jurisdiction depth to the explicit hierarchy after the hierarchy registry exists.
...
with base as (
  select c.id, c.iso_alpha2 as jurisdiction_key, c.country_name, c.country_slug, c.iso_alpha3,
         c.region, c.subregion, h.jurisdiction_level, c.market_access_status, c.medical_status, c.adult_use_status,
...
set status=case
 ...
 applicability=case when c.dimension_key in ('market_metrics','trade_flows','signals') and d.jurisdiction_level='subnational' then 'not_applicable' when c.dimension_key='country_intel' and d.jurisdiction_level='subnational' then 'inherited' else c.applicability end,
