...
declare
  ...
  v_batch_ok boolean;
begin
  -- A pg_net request with no response is unrecoverable; release its signals
  -- so the queue can retry rather than deadlocking indefinitely.
  ...
  loop
    ...
    -- Non-200 responses (including quota 429s) must not permanently block
    -- these signals. They remain eligible for a later dispatch.
...
as $function$
declare
  v_rid bigint; v_texts text[]; v_ids text[]; v_allowed int; v_requests jsonb; v_gemini_key text;
begin
  -- Prevent concurrent pipeline ticks from consuming the same Gemini quota.
  if not pg_try_advisory_xact_lock(hashtextextended('harbourview:gemini-embed-dispatch', 0)) then
    return null;
  end if;

  -- Respect Gemini's per-minute embedding quota after a 429 instead of
  -- immediately generating another guaranteed-to-fail request.
  if exists (
    select 1 from net._http_response
    where status_code = 429
      and created > now() - interval '65 seconds'
      and content::text like '%embed_content%'
  ) then
    return null;
  end if;

  -- Only one batch may be in flight at a time.
  if exists (select 1 from public.hv_embed_jobs where not harvested) then
    return null;
  end if;
  ...
  ) into v_requests from unnest(v_texts) t;
