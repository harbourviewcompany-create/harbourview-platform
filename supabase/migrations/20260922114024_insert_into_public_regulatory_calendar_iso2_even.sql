...
set status='verified_populated',applicability='applicable',evidence_basis='Primary source registry and current primary regulatory evidence/claim/pathway verified 2026-09-22.',last_evaluated_at=now(),notes='Primary-source regulatory layer reconciled.'
where jurisdiction_key in ('CA','KE') and dimension_key in ('source_registry','verified_regulatory_evidence','verified_regulatory_claims','verified_pathways','regulatory_calendar');

update public.jurisdiction_data_depth_tasks
set status='verified',updated_at=now(),notes=coalesce(notes,'')||' Reconciled from primary-source enrichment on 2026-09-22.'
where jurisdiction_key in ('CA','KE') and dimension_key in ('source_registry','verified_regulatory_evidence','verified_regulatory_claims','verified_pathways','regulatory_calendar');
