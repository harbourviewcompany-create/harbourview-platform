...
create policy genetics_evidence_items_approved_request_read on genetics_evidence_items for select using (exists (select 1 from genetics_access_requests gar join genetics_profiles gp on gp.id = gar.requester_profile_id where gar.cultivar_id = genetics_evidence_items.cultivar_id and gp.owner_user_id = auth.uid() and gar.status in ('approved_limited','approved_full') and genetics_evidence_items.visibility in ('registered_members','verified_members','qualified_buyers','nda_private','invite_only')));
