...
as $$
declare v_seen int:=0; v_upserted int:=0;
begin
  if coalesce(current_setting('request.jwt.claim.role',true),'') not in ('service_role','') and session_user not in ('postgres','supabase_admin') then
  ...
  with latest as (
    ...
    where sr.iso is not null and sr.iso<>''
  ...
  chosen as (
    select * from latest where rn=1 limit greatest(1,least(coalesce(p_limit,500),1000))
  )
  ...
  do update set evidence_kind=excluded.evidence_kind,
    applicability=excluded.applicability,evidence_payload=excluded.evidence_payload,
    evidence_quote=excluded.evidence_quote,source_registry_id=excluded.source_registry_id,
    source_snapshot_id=excluded.source_snapshot_id,source_url=excluded.source_url,
