...
alter table public.market_entry_missions
  add column if not exists payment_status text not null default 'unpaid' check (payment_status in ('unpaid','checkout_created','paid','failed','refunded')),
  add column if not exists report_status text not null default 'locked' check (report_status in ('locked','generating','ready','delivered','void')),
...
create table if not exists public.regulatory_market_access_claims (
  ...
  evidence_status text not null default 'unverified' check (evidence_status in ('unverified','partial','verified','superseded','rejected')),
  source_snapshot_sha256 text,
  source_snapshot_uri text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint market_entry_claim_freshness check (expires_at is null or verified_at is null or expires_at > verified_at)
);
...
create policy regulatory_market_access_claims_public_read on public.regulatory_market_access_claims for select to anon, authenticated using (evidence_status = 'verified' and source_snapshot_sha256 is not null and expires_at > now());
...
select c.claim_id,c.claim_key,c.evidence_key,c.jurisdiction_iso2,c.claim_text,c.product_class,c.jurisdiction_scope,c.authority_name,c.authority_url,c.source_document_id,c.source_effective_date,c.retrieved_at,c.verified_at,c.expires_at,c.source_snapshot_sha256,c.source_snapshot_uri
from public.regulatory_market_access_claims c
where c.evidence_status = 'verified' and c.authority_url <> '' and c.source_effective_date is not null and c.retrieved_at is not null and c.verified_at is not null and c.expires_at > now() and c.source_snapshot_sha256 is not null;
grant select on api.regulatory_market_access_executable_claims to authenticated, service_role;

insert into public.regulatory_market_access_claims (evidence_key,jurisdiction_iso2,claim_key,claim_text,product_class,jurisdiction_scope,authority_name,authority_url,source_effective_date,verified_at,expires_at,evidence_status,source_snapshot_sha256)
select e.evidence_key,e.jurisdiction_iso2,'market-access:' || e.evidence_key,e.rationale,'any',case when e.parent_iso2 is null then 'jurisdiction' else 'national_pathway_inheritance' end,e.authority_name,e.authority_url,e.source_effective_date,e.verified_at,e.expires_at,case when e.source_snapshot_sha256 is not null and e.source_effective_date is not null then 'verified' else 'partial' end,e.source_snapshot_sha256
