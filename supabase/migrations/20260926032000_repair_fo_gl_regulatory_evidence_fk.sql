...
select 'hv-mkt-complete-fo-20260913','FO','primary-evidence-claim:hv-mkt-complete-fo-20260913',
...
where exists (select 1 from public.regulatory_market_access_evidence e where e.evidence_key='hv-mkt-complete-fo-20260913')
on conflict (claim_key) do update set evidence_key=excluded.evidence_key,claim_text=excluded.claim_text,
authority_name=excluded.authority_name,authority_url=excluded.authority_url,
verified_at=excluded.verified_at,expires_at=excluded.expires_at,evidence_status='verified',updated_at=now();

insert into public.regulatory_market_access_claims(
 evidence_key,jurisdiction_iso2,claim_key,claim_text,product_class,jurisdiction_scope,
 authority_name,authority_url,source_effective_date,retrieved_at,verified_at,expires_at,evidence_status
)
select 'hv-mkt-complete-gl-20260913','GL','primary-evidence-claim:hv-mkt-complete-gl-20260913',
...
where exists (select 1 from public.regulatory_market_access_evidence e where e.evidence_key='hv-mkt-complete-gl-20260913')
on conflict (claim_key) do update set evidence_key=excluded.evidence_key,claim_text=excluded.claim_text,
authority_name=excluded.authority_name,authority_url=excluded.authority_url,
verified_at=excluded.verified_at,expires_at=excluded.expires_at,evidence_status='verified',updated_at=now();
