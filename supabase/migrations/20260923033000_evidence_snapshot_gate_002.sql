...
-- cryptographic hash, was fetched successfully, contains a payload, and
...
select
  ...
  case
    ...
    when ss.snapshot_hash !~ '^[0-9a-f]{64}$' then false
    when ss.fetched_at is null then false
    when ss.http_status is null or ss.http_status < 200 or ss.http_status >= 300 then false
    when ss.raw_payload is null or length(ss.raw_payload)=0 then false
...
with (security_invoker = on) as
select ss.id snapshot_id, ss.source_id, ss.snapshot_hash, ss.fetched_at,
       ss.http_status, sr.source_url registered_source_url,
       case
         when ss.snapshot_hash is null or ss.snapshot_hash !~ '^[0-9a-f]{64}$' then 'invalid_snapshot_hash'
         when ss.fetched_at is null then 'missing_fetched_at'
         ...
         when ss.raw_payload is null or length(ss.raw_payload)=0 then 'missing_raw_payload'
