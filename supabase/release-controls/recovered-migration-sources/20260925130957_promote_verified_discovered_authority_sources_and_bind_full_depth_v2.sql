...
-- Production migration version: 20260925130957
...
begin
  with candidates as (
    select distinct on (sr.iso, sr.regulator_class)
      sr.iso jurisdiction_iso2,'national' jurisdiction_level,sr.regulator_class source_class,
    ...
    where sr.iso is not null
      ...
      and sr.source_url like 'https://%'
      and sr.notes ilike '%source-discovery-engine%'
    ...
    order by sr.iso,sr.regulator_class,ss.captured_at desc
  ), ins as (
    ...
    from candidates c
    where not exists (
      select 1 from public.regulatory_market_access_primary_sources p
      where p.jurisdiction_iso2=c.jurisdiction_iso2
        and lower(regexp_replace(trim(p.authority_url), '/+$',''))=lower(regexp_replace(trim(c.authority_url), '/+$',''))
    ) returning 1
  ...
  with cb as (
    ...
    on conflict (jurisdiction_key,dimension_key,contract_version) do update set
      source_registry_id=excluded.source_registry_id,authority_url=excluded.authority_url,
      expected_snapshot_sha256=excluded.expected_snapshot_sha256,binding_status='bound',updated_at=now()
    returning 1
  ...
  where j.status='blocked' and j.contract_version='2026-09-23.v2'
    and exists (select 1 from public.jurisdiction_data_depth_authority_bindings b
      where b.jurisdiction_key=j.jurisdiction_key and b.dimension_key=j.dimension_key
      and b.contract_version=j.contract_version and b.binding_status='bound');

  return jsonb_build_object('primary_sources_inserted',inserted_primary,'bindings_upserted',inserted_bindings);
end;
$$;
revoke all on function public.promote_verified_discovered_authority_sources() from public,anon,authenticated;
grant execute on function public.promote_verified_discovered_authority_sources() to service_role;
select cron.schedule('full_depth_authority_source_promotion','*/5 * * * *','select public.promote_verified_discovered_authority_sources();');
