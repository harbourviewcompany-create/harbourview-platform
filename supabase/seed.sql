...
-- =============================================================================
-- Harbourview Platform — Development Seed Data
-- All names, URLs, and identifiers in the "Signal Engine V1" fixtures below are
-- fictional and for local dev only. Wrap in transaction so all-or-nothing on failure.
-- Idempotent with ON CONFLICT where applicable. Safe for repeated `supabase db reset`.
-- =============================================================================
...
-- =============================================================================
-- NEW (2026-07-08, fixed 2026-07-10): country_education_overlay + citation sources
-- =============================================================================
-- NOTE ON THIS FIX: the rows below were originally written against a column
-- shape (id text, topic_key, title, content_summary, key_actions,
-- compliance_notes, published_at, review_status='approved') that does not
-- match the actual `country_education_overlay` table. The real table (see
-- supabase/migrations/20260710140000_country_education_overlay.sql, restored
-- in this same fix — the CREATE TABLE migration was missing from this branch
-- entirely) has columns
-- (id uuid, country_iso2, module_key, role_id, topics jsonb, action_label,
-- source_ids uuid[], review_status, reviewer, last_verified_at, updated_at)
-- with a CHECK constraint on review_status that does not include 'approved'.
-- Rewritten below to match the real schema so this insert actually succeeds
-- on `supabase db reset`. role_id values ('importer', 'distributor',
-- 'operator') are kept as originally authored — role_id has no CHECK
-- constraint, and the live table already holds rows using yet another
-- role_id convention ('cultivator_producer', 'investor_operator', 'general'),
-- so there is no single enforced vocabulary to conform to; not worth
-- guessing at a "more correct" set. review_status is set to 'review_pending'
-- (not 'verified_*') because these citations were sourced for this fix but
-- not compliance/legal-reviewed — see docs/control/EVIDENCE_LOG.md.

-- ---------------------------------------------------------------------------
-- source_documents (4 rows) — real external citations backing the education
-- overlay claims below, referenced via source_ids. Unlike the fictitious
-- Signal Engine fixtures above, these cite real regulatory sources (verified
-- reachable/real via search at the time this fix was authored).
-- ---------------------------------------------------------------------------
DO $$ BEGIN
  INSERT INTO source_documents (
    id, source_url, source_title, source_domain,
  ...
  (
    ...
    'https://www.gesetze-im-internet.de/amg_1976/__72.html',
    'AMG §72 — Einfuhrerlaubnis (Import Licence for Medicinal Products), Germany',
    'gesetze-im-internet.de',
    'public_web', 'allowed', '2026-07-10',
    'Anyone wishing to import commercially certain substances for medicinal manufacture from non-EU/EEA countries requires an import licence (Einfuhrerlaubnis) under §72 AMG.',
    'Anyone wishing to import commercially certain substances for medicinal manufacture from non-EU/EEA countries requires an import licence (Einfuhrerlaubnis) under §72 AMG.',
    'retained', 'de',
    'devchecksum014amg72import',
    0.9800
  ),
  (
    '00000000-0014-0001-0001-000000000002',
    'https://www.gesetze-im-internet.de/btmg_1981/',
    'BtMG — Gesetz über den Verkehr mit Betäubungsmitteln (Narcotics Act), Germany',
    'gesetze-im-internet.de',
    'public_web', 'allowed', '2026-07-10',
    'The Betäubungsmittelgesetz regulates the manufacture, marketing, import, export, and permitted distribution channels (pharmacies, licensed distributors) for narcotics including medical cannabis.',
    'The Betäubungsmittelgesetz regulates the manufacture, marketing, import, export, and permitted distribution channels (pharmacies, licensed distributors) for narcotics including medical cannabis.',
    'retained', 'de',
    'devchecksum014btmg1981',
    0.9700
  ),
  (
    '00000000-0014-0001-0001-000000000003',
    'https://dof.gob.mx/nota_detalle.php?codigo=5609709&fecha=12/01/2021',
    'Reglamento de la Ley General de Salud en Materia de Investigación y Uso Médico de la Cannabis (DOF, 12 Jan 2021), Mexico',
    'dof.gob.mx',
    'public_web', 'allowed', '2026-07-10',
    'Establishes the COFEPRIS medical/research authorization pathway and the SENASICA National Registry of Cannabis Cultivation Permits for research and manufacturing purposes.',
    'Establishes the COFEPRIS medical/research authorization pathway and the SENASICA National Registry of Cannabis Cultivation Permits for research and manufacturing purposes.',
    'retained', 'es',
    'devchecksum014dofcannabis2021',
    0.9600
  ),
  (
    '00000000-0014-0001-0001-000000000004',
    'https://www.congress.gov/crs-product/R46709',
    'CRS Report R46709 — The Application of IRC Section 280E to Marijuana Businesses',
    'congress.gov',
    'public_web', 'allowed', '2026-07-10',
    'Congressional Research Service analysis of how IRC §280E denies ordinary business deductions to state-licensed marijuana businesses due to federal Schedule I/II status.',
    'Congressional Research Service analysis of how IRC §280E denies ordinary business deductions to state-licensed marijuana businesses due to federal Schedule I/II status.',
    'retained', 'en',
    'devchecksum014crsr46709',
    0.9500
  )
...
-- ---------------------------------------------------------------------------
-- country_education_overlay (4 rows — country & role specific modules)
-- ---------------------------------------------------------------------------
DO $$ BEGIN
  INSERT INTO country_education_overlay (
    id, country_iso2, role_id, module_key, topics, action_label,
    source_ids, review_status, last_verified_at
  ) VALUES
    -- Source: AMG §72 (Einfuhrerlaubnis) — see source_documents 0014-...0001
    (
      '00000000-0015-0001-0001-000000000001', 'DE', 'importer', 'compliance',
      '["EU-GMP certification mandatory for cannabis flower imports", "Annex 1 environmental monitoring and contamination control", "Import permit required under §72 AMG for non-EU/EEA sourcing", "Batch-level Certificate of Analysis and traceability logging required 5+ years"]'::jsonb,
      'Review BfArM/AMG import checklist',
      ARRAY['00000000-0014-0001-0001-000000000001']::uuid[],
      'review_pending', NOW() - INTERVAL '2 days'
    ),

    -- Source: BtMG (Betäubungsmittelgesetz) — see source_documents 0014-...0002
    (
      '00000000-0015-0001-0001-000000000002', 'DE', 'distributor', 'local_intel',
      '["Medical cannabis is dispensed only through licensed pharmacies and authorized distributors", "BtMG defines permitted narcotics classes, licensing and handling obligations", "Distributor authorization must be renewed and audited under BtMG oversight", "Recreational retail remains distinct from the medical pharmacy channel"]'::jsonb,
      'Build pharmacy/distributor network under BtMG',
      ARRAY['00000000-0014-0001-0001-000000000002']::uuid[],
      'review_pending', NOW() - INTERVAL '1 day'
    ),

    -- Source: DOF Reglamento (COFEPRIS + SENASICA) — see source_documents 0014-...0003
    (
      '00000000-0015-0001-0001-000000000003', 'MX', 'operator', 'genetics',
      '["High-CBD strains best suited to local growing conditions", "SENASICA cultivation permit required before planting/harvest", "COFEPRIS Research Protocol authorization is a prerequisite for the SENASICA permit", "Genetic origin and seed lineage must be documented for the National Registry"]'::jsonb,
      'File COFEPRIS + SENASICA registration',
      ARRAY['00000000-0014-0001-0001-000000000003']::uuid[],
      'review_pending', NOW() - INTERVAL '3 days'
    ),

    -- Source: CRS Report R46709 (IRC §280E) — see source_documents 0014-...0004
    (
      '00000000-0015-0001-0001-000000000004', 'US', 'operator', 'compliance',
      '["Cannabis remains federally controlled; state licensing governs day-to-day operations", "State seed-to-sale tracking (e.g. METRC or state-equivalent) is mandatory in licensed states", "IRC §280E disallows ordinary business deductions for federally scheduled substances", "State tax and compliance requirements vary significantly — track per-jurisdiction"]'::jsonb,
      'Model 280E tax exposure per state',
      ARRAY['00000000-0014-0001-0001-000000000004']::uuid[],
      'review_pending', NOW() - INTERVAL '4 days'
    )
  ON CONFLICT (id) DO NOTHING;
END $$;
