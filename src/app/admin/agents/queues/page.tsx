...
import { agentQueueItems } from '@/lib/fixtures/agents_fixtures'
...
export default async function QueuesPage() {
  ...
  const active = agentQueueItems.filter(i => ['pending', 'in_progress'].includes(i.status))
  ...
  const counts = {
    urgent: agentQueueItems.filter(i => i.priority === 'urgent').length,
    pending: agentQueueItems.filter(i => i.status === 'pending').length,
    in_progress: agentQueueItems.filter(i => i.status === 'in_progress').length,
    deferred: agentQueueItems.filter(i => i.status === 'deferred').length,
  }
  ...
  return (
    <div className="p-6 max-w-7xl space-y-6">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-zinc-800">
              {['Priority', 'Agent Type', 'Task', 'Market', 'Status', 'Next Action', 'Created'].map(h => (
          ...
          <tbody className="divide-y divide-zinc-800/50">
            {sorted.map(item => (
              <tr key={item.id} className="hover:bg-zinc-800/30 transition-colors">
                ...
                <td className="px-4 py-3 text-zinc-400 font-mono text-xs whitespace-nowrap">{item.agentType}</td>
                <td className="px-4 py-3 max-w-sm">
                  <div className="text-zinc-200 text-xs font-medium leading-relaxed">{item.title}</div>
                  <div className="text-zinc-500 text-xs mt-0.5">{item.market} · {item.category}</div>
                </td>
                <td className="px-4 py-3 text-zinc-400 text-xs">{item.market}</td>
                <td className="px-4 py-3">
      ...
      {agentQueueItems.filter(i => i.status === 'deferred').length > 0 && (
        <details className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
          <summary className="px-4 py-3 text-xs text-zinc-400 font-medium cursor-pointer hover:text-zinc-200 select-none">
            Deferred items ({agentQueueItems.filter(i => i.status === 'deferred').length})
          </summary>
          <div className="border-t border-zinc-800 divide-y divide-zinc-800/50">
            {agentQueueItems.filter(i => i.status === 'deferred').map(item => (
              <div key={item.id} className="px-4 py-3 flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="text-zinc-500 text-xs mt-0.5">{item.market} · {item.nextAction}</div>
                </div>
              </div>
            ))}
          </div>
