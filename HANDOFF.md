...
**5. `app/intelligence/licensing-pathways/page.tsx`, `app/intelligence/logistics-trade-routes/page.tsx`, and `app/genetics/cultivars/[slug]/page.tsx` — do not retire to Command Centre redirects**
The first two were rebuilt with real live-data wiring (licensing-pathways: cross-links to all 20 `jurisdiction_playbooks`; logistics-trade-routes: `corridor_processing_times` + `corridor_regulatory_alerts` benchmarks and alert feed) on Jul 2-3, after the #937 Command Centre consolidation branch had already retired both to 2-line redirect stubs. The third (`app/genetics/cultivars/[slug]/page.tsx`) predates the consolidation itself — it's the public cultivar passport detail page with a dedicated P0 security-boundary test suite (`tests/genetics/cultivarPassportNetwork.test.tsx`, run in CI as `test:genetics-profile-redaction`) verifying private evidence fields, buyer notes, and licensing terms never leak into the public payload. #937's blanket "retire all remaining standalone pages" commit swept this up without accounting for that test coverage; the Jul 3-4 rebase restored the live version and carved all three routes out of the consolidation. Do not silently re-retire any of them — if you want to fold them into Command Centre, build the equivalent CC panel first, confirm feature parity and test coverage, then update this note.
