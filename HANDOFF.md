...
| **Open PRs** | #1026 (migration drift reconciliation, partial — see ADR #17, gap is bigger than this PR states), #1024 (Market Entry OS audit, docs-only), #1023 (heatmap preferences guard, supersedes #1020), #1022 (compliance-brief rate limit, extracted from stale #990), #1021 (ratings schema forward-fix, migration-file-only — prod ALTER still pending explicit sign-off). All five show `check-drift` + `Enforce registry impact discipline` failing — both are known pre-existing checks, not caused by these PRs. |
