...
def run_embedded_workflow_python(path: str) -> None:
...
# Stable application/security/documentation assembler through V4.
run_embedded_workflow_python('.github/workflows/repair-production-readiness-v4.yml')

source_intake_sql = textwrap.dedent('''\
...
''')
Path('supabase/migrations/20260305000000_live_source_intake_v0_consumables.sql').write_text(source_intake_sql)

source_intake_forward = source_intake_sql + textwrap.dedent('''\
...
''')
Path('supabase/migrations/20260804235000_separate_marketplace_source_intake.sql').write_text(source_intake_forward)

for code_path in ['lib/marketplace/liveSources.ts', 'lib/marketplace/candidates.ts']:
    text = Path(code_path).read_text()
    text = text.replace('/rest/v1/source_registry', '/rest/v1/marketplace_source_registry')
    text = text.replace('/rest/v1/source_snapshots', '/rest/v1/marketplace_source_snapshots')
    text = text.replace('/rest/v1/candidate_review_events', '/rest/v1/marketplace_candidate_review_events')
    Path(code_path).write_text(text)


def correct_source_import(path_value: str) -> str:
    migration_path = Path(path_value)
    sql = migration_path.read_text()
    if 'drop table if exists pg_temp.tmp_source_import_upserted;' not in sql:
        sql = sql.replace(
    ...
    if 'create temp table tmp_source_import_upserted on commit drop as' in sql:
        raise RuntimeError(f'{path_value}: invalid CREATE TABLE AS INSERT syntax remains')
    migration_path.write_text(sql)
    return sql


correct_source_import('supabase/migrations/20260526000000_phase1_source_import_recovery.sql')
canonical_source_import = correct_source_import('supabase/migrations/20260526030100_phase1_source_import_recovery.sql')
function_start = canonical_source_import.index('create or replace function public.import_source_registry_batch')
function_end = canonical_source_import.index('alter table public.source_import_batches enable row level security;', function_start)
Path('supabase/migrations/20260804235500_fix_source_import_batch_function.sql').write_text(
    'begin;\n\n' + canonical_source_import[function_start:function_end] + 'commit;\n'
)

country_schema = textwrap.dedent('''\
...
''')
Path('supabase/migrations/20260604000000_countries_public_table_v1.sql').write_text(
    '-- Deterministic replay-safe countries foundation. Data seeding remains in 20260609000000.\n' + country_schema
)
Path('supabase/migrations/20260805000000_countries_table_replay_repair.sql').write_text(
    '-- Forward, idempotent reconciliation for production histories that passed the old timestamp.\n' + country_schema
)

print('Stable production-readiness candidate tree assembled.')
