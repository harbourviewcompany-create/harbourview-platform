...
PY
  then pass_check "active main ruleset matches the canonical governance policy"
  else fail_check "main ruleset is missing, incomplete, or unauthorized"
  fi
else
...
if [ "$mutable_refs" -eq 0 ]; then
  pass_check "all external workflow actions and reusable workflows are pinned to immutable commit SHAs"
else
  fail_check "$mutable_refs external workflow action/reusable-workflow reference(s) are mutable"
fi

if grep -RInE '^[[:space:]]*pull_request_target[[:space:]]*:' "$workflow_dir" >/tmp/governance-pull-target.txt 2>/dev/null; then
  cat /tmp/governance-pull-target.txt; fail_check "pull_request_target is prohibited in repository workflows"
else
  pass_check "no pull_request_target workflow is present"
fi

if grep -RInE 'permissions:[[:space:]]*write-all|^[[:space:]]+permissions:[[:space:]]*write-all' "$workflow_dir" >/tmp/governance-write-all.txt 2>/dev/null; then
  cat /tmp/governance-write-all.txt; fail_check "permissions: write-all is prohibited"
else
