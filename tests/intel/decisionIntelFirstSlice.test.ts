...
const tierAuthority = readFileSync('lib/stripe/tier.ts', 'utf8')
...
describe('Decision Intelligence Stage 0 first slice', () => {
  ...
  it('uses user_profiles.tier as the single Decision Intel entitlement authority', () => {
    expect(tierAuthority).toContain(".from('user_profiles')")
    expect(tierAuthority).toContain(".select('tier')")
    expect(dashboardPage).toContain('getUserTier()')
