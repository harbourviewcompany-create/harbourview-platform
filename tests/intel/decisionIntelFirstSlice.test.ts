...
describe('Decision Intelligence Stage 0 first slice', () => {
  ...
  it('preserves evidence relationships, including event-level contradictions, and canonical ownership for suppressed rows', () => {
    ...
    expect(dossierLoader).toContain('Canonical ownership without an allowlisted dossier row used to return null')
    expect(dossierLoader).toContain('fall through to legacy so the operator can still')
    expect(dossierLoader).toContain('if (!isSyntheticEventRoute) return null')
  })
  ...
  it('uses user_profiles.tier as the single Decision Intel entitlement authority', () => {
    ...
    // The page resolves the canonical profile tier before wiring entitlement checks.\n    expect(dashboardPage).toMatch(/(?:userTier|resolvedUserTier)\s*=\s*await getUserTier\(\)/)
    ...
    expect(desktopShell).toContain('access={decisionIntelAccess}')
    expect(responsiveShell).toContain('decisionIntelAccess?: FeatureAccess')
    expect(responsiveShell).toContain('{...props}')
  ...
  it('keeps editorial, digest, story and research rows out of synthetic dossier routes while preserving eligible digest routes', () => {
    ...
    expect(desktopShell).toContain('[...signals, ...(digestSignals ?? [])]')
  ...
  it('makes mobile and desktop intelligence reach the dossier without losing dashboard context', () => {
    ...
    expect(intelUi).toContain('className="hvm2-intel-cta"')
