...
describe('GET /api/used-surplus-preview', () => {
  ...
  it('includes DTO-safe candidates only when includeCandidates=true', async () => {
    ...
    expect(Object.keys(candidate).sort()).toEqual(allowedKeys)
  })

  it('prevents forbidden public leakage strings in serialized payload', async () => {
    const { response, body } = await callRoute('?includeCandidates=true&status=fetched&limit=1')
    expect(response.status).toBe(200)
