...
describe('evidence architecture hardening 006', () => {
  test('requires dimension-aware authority policy', () => {
    const sql=String.raw`-- Evidence architecture hardening 006.
...
`;
    expect(sql).toContain('jurisdiction_data_depth_dimension_authority_requirements');
    expect(sql).toContain('requires_dimension_specific_source');
  });
  test('requires evidenced applicability and qualifying snapshots', () => {
    const sql=String.raw`-- Evidence architecture hardening 006.
-- Adds dimension-aware authority requirements, applicability evidence,
-- snapshot/source binding, freshness/effective-date checks, conflict
-- resolution signals, immutable lineage diagnostics, and explicit gate codes.
...
comment on table public.jurisdiction_data_depth_applicability_evidence is
  'Evidence-backed applicability decisions. Not-applicable is not a completeness escape hatch.';
`;
    expect(sql).toContain('MISSING_APPLICABILITY_EVIDENCE');
    expect(sql).toContain('APPLICABILITY_SNAPSHOT_INVALID');
    expect(sql).toContain('basis_type');
  });
  test('checks exact snapshot/source binding and effective dates', () => {
    const sql=String.raw`-- Evidence architecture hardening 006.
-- Adds dimension-aware authority requirements, applicability evidence,
-- snapshot/source binding, freshness/effective-date checks, conflict
-- resolution signals, immutable lineage diagnostics, and explicit gate codes.
...
comment on table public.jurisdiction_data_depth_applicability_evidence is
  'Evidence-backed applicability decisions. Not-applicable is not a completeness escape hatch.';
`;
    expect(sql).toContain('SNAPSHOT_SOURCE_MISMATCH');
    expect(sql).toContain('FUTURE_EFFECTIVE');
    expect(sql).toContain('EXPIRED');
  });
  test('exposes structured gate reasons', () => {
    const sql=String.raw`-- Evidence architecture hardening 006.
-- Adds dimension-aware authority requirements, applicability evidence,
-- snapshot/source binding, freshness/effective-date checks, conflict
-- resolution signals, immutable lineage diagnostics, and explicit gate codes.
...
comment on table public.jurisdiction_data_depth_applicability_evidence is
  'Evidence-backed applicability decisions. Not-applicable is not a completeness escape hatch.';
`;
    expect(sql).toContain('v_jurisdiction_full_depth_gate_reasons');
