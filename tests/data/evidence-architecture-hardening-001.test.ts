...
describe('evidence architecture hardening 001', () => {
  ...
  test('does not silently upgrade existing evidence', () => {
    expect(sql).toContain('Phase 1 is intentionally non-destructive')
    expect(sql).toContain('historical verified rows may predate')
    expect(sql).toContain('Future writes are fail-closed')
  })
})
