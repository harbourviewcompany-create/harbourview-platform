...
describe('evidence capture queue 005', () => {
  test('queues verified evidence without qualifying snapshots', () => {
    const sql = String.raw`-- Evidence architecture hardening 005.
...
`;
    expect(sql).toContain('v_jurisdiction_evidence_capture_queue');
    expect(sql).toContain("verification_status='verified'");
    expect(sql).toContain('not coalesce(g.qualifying_snapshot,false)');
  });
  test('matches registry by exact source URL and never invents provenance', () => {
    const sql = String.raw`-- Evidence architecture hardening 005.
-- Operational capture queue for verified facts that still lack a qualifying immutable snapshot.
-- This migration creates no evidence and never auto-links a fact to an unrelated snapshot.
...
comment on view public.v_jurisdiction_evidence_capture_queue is
  'Operational queue for verified structured evidence lacking a qualifying immutable source snapshot. It identifies capture work but never fabricates or auto-links provenance.';
`;
    expect(sql).toContain('sr.source_url=r.source_url');
    expect(sql).toContain('never fabricates or auto-links provenance');
  });
});
