...
describe('publicSafety forbidden list (audit P2-5)', () => {
  ...
  it('rejects contactEmail', () => { expect(() => assertPublicSafe({ id: '1', contactEmail: 'x@y.z' })).toThrow(/forbidden/i) })
  it('allows confidence_score', () => { expect(() => assertPublicSafe({ id: '1', confidence_score: 80, title: 'ok' })).not.toThrow() })
  it('finds forbidden fields recursively', () => { expect(findForbiddenPublicFieldNames({ outer: { nested: { private_notes: 'secret' } } })).toContain('private_notes') })
})
