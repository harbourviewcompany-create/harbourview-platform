...
describe('commercial country dashboard payload boundary', () => {
  it('projects public-safe commercial dashboard structures without forbidden private keys', () => {
    ...
    expect(commercial.marketplace.listings[0]).toMatchObject({
      publicSummary: expect.any(String),
      buyerFit: expect.any(Array),
      href: expect.stringContaining('/marketplace/listings/'),
    })
  })

  it('detects nested forbidden public dashboard keys and legacy layer aliases', () => {
    expect(assertCountryDashboardPublicSafe({ nested: { raw_source_urls: ['private'], reviewerFields: 'private' } })).toEqual(['raw_source_urls', 'reviewerFields'])
    expect(normalizeCountryHeatmapLayer('regulatory_signals')).toBe('regulatory_status')
    expect(normalizeCountryHeatmapLayer('import_potential')).toBe('import_export_fit')
  })
})
...
describe('commercial country dashboard render views', () => {
  it('renders Brazil Buyer view with marketplace workflows visible first', () => {
    ...
    expect(html).toContain('/marketplace/listings/soft-launch')
  ...
  it('renders a fallback country view with public-safe commercial framing', () => {
    ...
    expect(html).toContain('Selected layer summary')
  })
})
