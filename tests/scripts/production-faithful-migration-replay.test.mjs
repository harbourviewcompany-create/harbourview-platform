...
test('replay restores the verified jurisdiction-level projection before depth coverage consumers', () => {
  const file = '20260922101323_jurisdiction_data_depth_v1.sql'
  const patch = contentPatches.find((item) => item.file === file)
  assert.ok(patch)
  assert.equal(patch.anchor, "         c.data_completeness, c.last_updated_label,\n")
  assert.equal(
    patch.replacement,
...
test('replay compares market_access_status as text so either column shape works', () => {
 ...
 then 'subnational' else 'national' end as jurisdiction_level,\n",
  )
  const original = fs.readFileSync(path.join(root, 'supabase/migrations', file), 'utf8')
  assert.equal(original.includes(patch.anchor), true)
  assert.equal(original.includes(patch.replacement), false)
