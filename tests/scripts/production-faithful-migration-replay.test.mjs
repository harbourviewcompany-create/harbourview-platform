...
test('replay restores the jurisdiction-level projection before depth coverage consumers', () => {
  const file = '20260922101323_jurisdiction_data_depth_v1.sql'
  const patch = contentPatches.find((item) => item.file === file)
  assert.ok(patch)
  assert.match(patch.anchor, /data_completeness, c\.last_updated_label/)
  assert.match(patch.replacement, /jurisdiction_level/)
})
