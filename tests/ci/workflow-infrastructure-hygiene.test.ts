...
describe('CI infrastructure hygiene', () => {
  ...
  it('keeps E2E out of the PR CI path unless an explicit main-only job is reintroduced', () => {
    const ci = read('.github/workflows/ci.yml')

    expect(ci).not.toContain('  e2e:')
    expect(ci).not.toContain('npm run test:e2e')
    expect(ci).not.toContain('npx playwright install --with-deps chromium')
    expect(ci).not.toContain('name: Check optional E2E credentials')
    expect(ci).not.toContain('Supabase E2E readiness')
  })})
