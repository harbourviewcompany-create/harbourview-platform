...
test.describe('Decision Intelligence first slice', () => {
  test('mobile Intel rows drill into evidence-backed dossiers at required widths', async ({ browser }) => {
    ...
    for (const viewport of VIEWPORTS) {
      ...
      try {
        const page = await context.newPage()
        const response = await page.goto('/dashboard?country=CA&role=exporter&section=weekly-signals&page=signals', { waitUntil: 'domcontentloaded', timeout: 60_000 })
  ...
  test('desktop dossier preserves the decision/evidence hierarchy', async ({ browser }) => {
    ...
    try {
      const page = await context.newPage()
      await page.goto('/dashboard?country=CA&role=exporter&section=weekly-signals&page=signals', { waitUntil: 'domcontentloaded' })
      const dossierLink = page.getByRole('link', { name: /Open intelligence dossier:/ }).first()
      await expect(dossierLink).toBeVisible()
      await dossierLink.click()
