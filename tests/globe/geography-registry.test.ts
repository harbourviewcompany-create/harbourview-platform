...
import { COUNTRIES, getCountryByIso, getCountryCenter, getCountriesByRegion } from '@/data/globe/geography-registry'
...
describe('ISO-3166 geography registry', () => {
  ...
  it('preserves known repository geography labels without turning them into regulatory evidence', () => {
    expect(getCountryByIso('CA')?.region).toBe('Americas')
    expect(getCountriesByRegion('Americas').length).toBeGreaterThan(0)
  })
