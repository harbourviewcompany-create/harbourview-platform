...
## Migration Discipline — Applying Is Half the Change
- A migration applied to production without an identical file committed to `supabase/migrations/` is an incomplete change, not a completed one. This holds however it was applied: Supabase MCP `apply_migration`, the dashboard SQL editor, `psql`, or a workflow.
- The two halves happen in the same session. "I'll commit the file after" is how the drift accumulates — measured on 2026-09-02 at 30 uncommitted production versions, 25 of them applied through Supabase MCP by a single account.
- The `Migration Drift Check` workflow is the detector for this and it is load-bearing. It runs hourly against the live ledger and reports `applied_not_committed`. **A red drift gate is a broken change-control loop, not background noise** — it had been failing continuously since 2026-08-30 before anyone looked, because nothing downstream actually blocks on it.
- Recovery after the fact is possible but lossy: `supabase_migrations.schema_migrations.statements` holds the applied SQL verbatim and should always be the source for a reconstructed file (never re-derive SQL from current schema state — see `docs/control/MIGRATION_DRIFT_RECONCILIATION_20260810.md`). Reconstruct with the original `version`, and preserve the original comments: on PR #1740 a reconstruction silently dropped 848 bytes of jurisdiction-classification rationale while keeping the SQL byte-identical.
