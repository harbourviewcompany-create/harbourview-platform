...
export async function POST(request: Request) {
  try {
    ...
    const parsedIntake = baseIntakeSchema.safeParse({
      ...
      honeypot: readField(body, 'website'),
    })

    if (!parsedIntake.success) {
      return json('error', withCode('Please complete all required listing submission fields.', 'LISTING_SUBMISSION_VALIDATION_REQUIRED_FIELDS'), 400)
    }
    ...
    if (!assertNoHoneypot(honeypot)) {
      return json('success', withCode('Listing submission received. Harbourview will review it before publication or counterparty routing.', 'LISTING_SUBMISSION_OK'))
    }

    const textFields = [name, email, company, listingType, title, price, location].map((field) => normalizeFreeText(field))
    if (textFields.some((field, i) => i >= 0 && field.length === 220)) {
