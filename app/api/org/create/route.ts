...
export async function POST(req: NextRequest) {
  ...
  const slug = (trade_name?.trim() || legal_name.trim()).toLowerCase().replace(/[^a-z0-9\s-]/g,"").replace(/\s+/g,"-").replace(/-+/g,"-").slice(0,48) + "-" + Math.random().toString(36).slice(2,7);
  const supabase = await createSupabaseServiceClient();
  const { data: existing } = await supabase.from("workspace_members").select("workspace_id").eq("user_id", user.id).eq("role","owner").eq("status","active").single();
  if (existing) return NextResponse.json({ error: "USER_ALREADY_HAS_ORG", org_id: existing.workspace_id }, { status: 409 });
  const { data: ws, error: wsErr } = await supabase.from("workspaces").insert({ name: trade_name?.trim()||legal_name.trim(), slug, legal_name: legal_name.trim(), trade_name: trade_name?.trim()||null, org_type, jurisdiction_country: jurisdiction_country.trim().toUpperCase().slice(0,2), jurisdiction_region: jurisdiction_region?.trim()||null, verification_status:"unverified", is_public:false, settings:{}, status:"active" }).select("id,slug,name").single();
  if (wsErr||!ws) { if (wsErr?.code==="23505") return NextResponse.json({ error: "SLUG_CONFLICT" }, { status: 409 }); return NextResponse.json({ error: "CREATE_FAILED" }, { status: 500 }); }
  const { error: mErr } = await supabase.from("workspace_members").insert({ workspace_id:ws.id, user_id:user.id, role:"owner", status:"active", invited_at:new Date().toISOString(), joined_at:new Date().toISOString() });
  ...
  await supabase.from("audit_events").insert({ entity_type:"workspace", entity_id:ws.id, action:"org.created", actor:user.id, actor_user_id:user.id, actor_org_id:ws.id, metadata:{ org_type, jurisdiction_country } });
