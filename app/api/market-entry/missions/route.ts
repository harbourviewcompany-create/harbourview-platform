...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select('id, origin_iso2, destination_iso2, product_class, status, plan_version, reproducibility_key, payment_status, report_status, created_at, updated_at')
...
export async function POST(request: NextRequest) {
  ...
  const { data: mission, error: missionError } = await supabase
    .from('market_entry_missions')
    .insert({ owner_user_id: user.id, origin_iso2: plan.input.originIso2, destination_iso2: plan.input.destinationIso2, product_class: plan.input.productClass, status: plan.status, plan_version: plan.planVersion, plan_snapshot: plan, evidence_snapshot: plan.evidence, reproducibility_key: plan.reproducibilityKey })
    .select('id, origin_iso2, destination_iso2, product_class, status, plan_version, reproducibility_key, payment_status, report_status, created_at, updated_at')
    .single()
  ...
  const taskRows = plan.execution.tasks.map((task) => ({ mission_id: mission.id, task_key: task.id, title: task.title, description: task.description, side: task.side, status: task.status, prerequisite_task_keys: task.prerequisiteIds, estimated_weeks: task.estimatedWeeks, evidence_ids: task.evidenceIds }))
  if (taskRows.length) {
  ...
  const { error: eventError } = await supabase.from('market_entry_events').insert({ mission_id: mission.id, actor_user_id: user.id, event_type: 'mission_created', to_status: plan.status, payload: { planVersion: plan.planVersion, reproducibilityKey: plan.reproducibilityKey } })
  if (eventError) {
    await supabase.from('market_entry_missions').delete().eq('id', mission.id)
