...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    ...
    .select("id,queue_type,target_entity_type,target_entity_id,org_id,assigned_to,priority,status,notes,created_at,updated_at,resolved_at,workspaces:org_id(id,slug,trade_name,legal_name,org_type,jurisdiction_country,verification_status)")
...
export async function PATCH(req: NextRequest) {
  ...
  if (status) {
    ...
    if (["resolved", "escalated"].includes(status)) update.resolved_at = new Date().toISOString()
  }
  ...
  await supabase.from("audit_events").insert({
    entity_type: "admin_review_queue", entity_id: queue_id,
    action: "admin_queue.updated", actor: user.id, actor_user_id: user.id, metadata: { status },
  })
