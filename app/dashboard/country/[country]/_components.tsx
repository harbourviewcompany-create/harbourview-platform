...
import type { CountryDashboardRole, CountryDashboardSummary } from '@/lib/dashboard/contracts'
...
export function CountryDashboardShell({
  ...
  const activeLayer = normalizeCountryHeatmapLayer(selectedLayer)
  ...
  const selectedLayerSummary = `${countryHeatmapLayerLabels[activeLayer]} is scored at ${selectedMetric.score}/100 for ${dto.displayName}. ${selectedMetric.badge} countries can be compared before selecting a transaction, education, readiness, or review path.`
            ...
            <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
              <div className="grid gap-5 lg:grid-cols-[1fr_300px]">
                <div>
                  ...
                  <div className="mt-5 rounded-2xl border border-[#c6a55a]/20 bg-[#07111f]/70 p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-[#c6a55a]">Selected layer summary</p>
                    <p className="mt-2 text-sm leading-6 text-white/65">{selectedLayerSummary}</p>
                  </div>
                  <div className="mt-5 grid gap-2" aria-label="Comparison legend">
              ...
              <section className="grid gap-4 md:grid-cols-2" aria-label="Marketplace first-screen cards">
                {commercial.marketplace.listings.map((listing) => (
                  <article key={listing.id} className="rounded-2xl border border-[#c6a55a]/20 bg-white/[0.035] p-4">
                    ...
                    <p className="mt-2 text-sm text-white/62">{listing.category} · {listing.listingTypeLabel} · {listing.locationLabel}</p>
                    <p className="mt-3 text-sm leading-6 text-white/65">{listing.publicSummary}</p>
                    <p className="mt-3 text-xs text-white/55"><span className="text-[#c6a55a]">Commercial path:</span> {listing.availabilityLabel}</p>
                    {listing.priceLabel ? <p className="mt-2 text-xs text-white/55"><span className="text-[#c6a55a]">Pricing:</span> {listing.priceLabel}</p> : null}
                    <div className="mt-3 flex flex-wrap gap-2">
                      {listing.buyerFit.map((fit) => <span key={fit} className="rounded-full border border-[#c6a55a]/20 px-2.5 py-1 text-[11px] text-white/60">{fit}</span>)}
                    </div>
