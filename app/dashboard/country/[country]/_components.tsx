    ...
    <main className="min-h-screen bg-[#03070d] text-white">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-4 py-5 md:px-6 lg:px-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[285px_1fr_230px]">
          <aside className="hidden rounded-3xl border border-white/10 bg-white/[0.03] p-4 lg:block" aria-label="Country index and role navigation">
            ...
            <CountrySwitcher id="country-switcher" currentSlug={dto.slug} countries={countries.map((item) => ({ slug: item.slug, displayName: item.displayName, dashboardPath: item.dashboardPath }))} />
          ...
          <section className="min-w-0 space-y-5">
            ...
            <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
              <div className="grid gap-5 lg:grid-cols-[1fr_300px]">
                <div>
                  ...
                  <div className="mx-auto mt-4 rounded-3xl border border-[#c6a55a]/30 bg-[#07111f]/92 p-4 text-center shadow-[0_18px_50px_rgba(0,0,0,0.36)] lg:hidden" aria-label="Mobile country and role selector">
                    <p className="text-[11px] uppercase tracking-[0.22em] text-[#c6a55a]">Country + role</p>
                    <div className="mt-3 grid gap-3">
                      <label className="text-left text-xs uppercase tracking-[0.16em] text-white/50" htmlFor="mobile-country-switcher">Country</label>
                      <CountrySwitcher id="mobile-country-switcher" currentSlug={dto.slug} countries={countries.map((item) => ({ slug: item.slug, displayName: item.displayName, dashboardPath: item.dashboardPath }))} />
                      <div className="text-left">
                        <p className="mb-2 text-xs uppercase tracking-[0.16em] text-white/50">Role</p>
                        <div className="grid max-h-44 gap-2 overflow-y-auto pr-1">
                          {countryDashboardRoles.map((item) => (
                            <Link key={`mobile-${item}`} href={`${dto.dashboardPath}?role=${item}&layer=${activeLayer}`} className={`rounded-xl border px-3 py-2 text-sm ${item === role ? 'border-[#c6a55a]/60 bg-[#c6a55a]/10 text-[#f4d27a]' : 'border-white/10 text-white/70'}`}>{countryDashboardRoleLabels[item]}</Link>
                          ))}
                        </div>
                      </div>
                    </div>
                    <p className="mt-3 text-xs leading-5 text-white/55">{countryHeatmapLayerLabels[activeLayer]} score: <span className="text-[#f4d27a]">{selectedMetric.score}</span>. Use the globe below for comparison.</p>
                  </div>
            ...
            <section className="grid gap-4 md:grid-cols-4">
              ...
              <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><h3 className="font-semibold">Movement / Signals</h3><p className="mt-2 text-sm leading-6 text-white/62">{commercial.movement.summary}</p></article>
            </section>
          </section>
