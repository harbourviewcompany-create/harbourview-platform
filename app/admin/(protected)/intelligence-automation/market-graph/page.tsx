...
import { listIaGraphEntities, listIaGraphEdges } from '@/lib/intelligence-automation/db';
...
export default async function IntelGraphPage() {
  ...
  const isLive   = entitiesResult.ok && edgesResult.ok;
  ...
  const totalConnections = entities.reduce((sum, e) => sum + e.connectionCount, 0);
  const evidencedEdges   = edges.filter(e => e.evidenced).length;
  ...
  return (
    <section className="space-y-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-[#F5F1E8]/65">
            Entities and edges connecting markets, counterparties, pathways, signals, categories, and sources.{' '}
          ...
          <tbody className="divide-y divide-white/8">
            {entities.map(e => (
              <tr key={e.id} className="text-[#F5F1E8]/75 hover:bg-white/[0.02]">
                ...
                <td className="p-4">
                  <span className={`rounded-full border px-2 py-0.5 text-[10px] capitalize ${typeColors[e.type] ?? 'text-[#F5F1E8]/50 border-white/15'}`}>
                    {e.type.replace(/_/g, ' ')}
                  </span>
                </td>
      ...
      {/* Edges table */}
      <div>
        <h3 className="mb-3 text-xs uppercase tracking-[0.18em] text-[#C6A55A]/70">
          Graph edges ({edges.length}) — {evidencedEdges} evidenced
        </h3>
            ...
            <tbody className="divide-y divide-white/8">
              {edges.map(e => (
                <tr key={e.id} className="text-[#F5F1E8]/70 hover:bg-white/[0.02]">
                  ...
                  <td className="p-4">
                    <span className={`text-xs font-semibold ${strengthColors[e.strength]}`}>{e.strength}</span>
                  </td>
                  <td className="p-4 text-xs">
                    {e.evidenced
                      ? <span className="text-emerald-400">Yes</span>
                      : <span className="text-[#F5F1E8]/35">No</span>}
                  </td>
                </tr>
              ))}
            </tbody>
