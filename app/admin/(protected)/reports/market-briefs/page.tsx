...
export default async function MarketBriefsPage() {
  await requireAdminAuth()
  const result = await listInstitutionalReports()
  const allReports = result.ok ? result.data : []
  const reports = allReports.filter((r) => r.report_type === 'market_brief')
  ...
  return (
    <section className="space-y-8">
      ...
      <div>
        ...
        <h2 className="mt-1 text-2xl font-semibold text-[#F5F1E8]">Market Briefs</h2>
        <p className="mt-2 text-sm text-[#F5F1E8]/60">
          Institutional market briefs covering regulatory pathways, buyer demand, and commercial opportunities.
        </p>
      </div>
