...
export default async function OpportunityMemosPage() {
  await requireAdminAuth()
  const result = await listInstitutionalReports()
  const allReports = result.ok ? result.data : []
  const reports = allReports.filter((r) => r.report_type === 'opportunity_memo')
  ...
  return (
    <section className="space-y-8">
      ...
      <div>
        ...
        <h2 className="mt-1 text-2xl font-semibold text-[#F5F1E8]">Opportunity Memos</h2>
        <p className="mt-2 text-sm text-[#F5F1E8]/60">
          Bespoke opportunity analysis memos for specific counterparties, markets, and categories.
        </p>
      </div>
