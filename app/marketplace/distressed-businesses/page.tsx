...
export const metadata: Metadata = {
  title: 'Distressed Businesses & Assets | Harbourview',
  description:
    'Reviewed acquisition and investment opportunities in distressed cannabis businesses, facilities and operating assets. Confidential review before any counterparty access.',
}

const acquisitionTypes = [
  { title: 'Distressed businesses', body: 'Licensed operators, retail, cultivation, processing or distribution businesses where restructuring, acquisition or transition is being considered.' },
  { title: 'Distressed facilities', body: 'Licensed production, processing, or retail facilities available through closure, lease assignment, or asset sale.' },
  { title: 'Licence and permit assets', body: 'Licensed entities, permit portfolios and regulatory assets in qualifying jurisdictions, reviewed for transfer eligibility.' },
  { title: 'Investor and acquirer matching', body: 'Harbourview-reviewed matching for investors, acquirers and transition-ready operators seeking confidential introductions.' },
]

function ListingCard({ listing }: { listing: PublicListing }) {
  const specs = listing.high_level_specs as Record<string, unknown>
  const ctaLabel = (specs?.cta_label as string) ?? 'Request qualified access'
  return (
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="mb-2 text-lg font-semibold text-[#f5f1e8]">No public opportunities available</p>
      <p className="mb-6 text-sm leading-7 text-white/54">Distressed business opportunities are handled confidentially. Speak with Harbourview directly.</p>
      <Link href="/intake" className="btn-marketplace text-sm">Speak confidentially</Link>
    </div>
  )
}

export default async function DistressedBusinessesPage() {
  const listings = await getPublicListingsByCategory('distressed_businesses')
  return (
    <>
      <section className="relative overflow-hidden border-b border-gold/10 bg-[#061120] py-14 text-white sm:py-16 lg:py-20">
        ...
        <div className="page-container relative z-10">
          ...
          <div className="mt-6 max-w-3xl text-base leading-8 text-white/62 sm:text-lg">
            <p>Acquisition and investment opportunities in licensed cannabis businesses, facilities and regulatory assets. All submissions reviewed privately. Counterparty identity and diligence materials disclosed only after qualification.</p>
          </div>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            ...
            <Link href="/marketplace/qualified-access" className="btn-intelligence min-h-[52px] justify-center text-center text-sm">Request qualified access</Link>
          </div>
        </div>
      </section>
      <section className="border-b border-gold/10 bg-[#030b16] py-10">
        <div className="page-container grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
          {acquisitionTypes.map((t) => (
            <div key={t.title} className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">{t.title}</h3>
              <p className="text-sm leading-7 text-white/52">{t.body}</p>
            </div>
          ))}
        </div>
      </section>
      <section className="border-b border-gold/10 bg-[#020814] py-8">
        <div className="page-container">
          <div className="rounded-sm border border-gold/20 bg-[linear-gradient(180deg,rgba(10,20,35,0.94)_0%,rgba(5,12,22,0.98)_100%)] p-6">
            <p className="mb-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-gold/72">Confidential intake — sanitised summaries only</p>
            <p className="text-sm leading-7 text-white/62">Party identity, financial details, distress context and diligence materials are never disclosed publicly. Qualified access requires Harbourview review and approval by the relevant parties.</p>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-gold/10 bg-[#020814] py-12 sm:py-16">
        <div className="page-container flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.26em] text-gold/72">Representing a distressed business or facility?</p>
            <h2 className="font-serif text-3xl leading-tight tracking-[-0.035em] text-[#f5f1e8] sm:text-4xl">Submit for confidential review.</h2>
            <p className="mt-4 text-sm leading-7 text-white/58">Licensed businesses, facilities, permit portfolios and operating assets. Review and counterparty qualification before any introduction.</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <Link href="/intake" className="btn-marketplace min-h-[52px] justify-center text-center text-sm">Submit confidentially</Link>
            <Link href="/marketplace/qualified-access" className="btn-intelligence min-h-[52px] justify-center text-center text-sm">Qualified access</Link>
          </div>
        </div>
      </section>
    </>
