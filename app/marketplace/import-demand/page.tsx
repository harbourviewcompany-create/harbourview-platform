...
export const metadata: Metadata = {
  title: 'Import Demand | Harbourview',
  description: 'Reviewed import demand requests for regulated cannabis. Buyer and importer procurement mandates routed through Harbourview review before any supplier introduction.',
}
...
function ListingCard({ listing }: { listing: PublicListing }) {
  ...
  return (
    <div className="group flex flex-col rounded-sm border border-gold/10 bg-[linear-gradient(180deg,rgba(10,20,35,0.94)_0%,rgba(5,12,22,0.98)_100%)] p-6 shadow-[0_18px_44px_rgba(0,0,0,0.22)] transition-all duration-200 hover:border-gold/30">
      ...
      <Link href={`/contact?ref=${listing.slug ?? listing.id}&type=import-demand`} className="btn-marketplace mt-6 justify-center text-center text-sm">{ctaLabel}</Link>
    </div>
  )
}
...
export default async function ImportdemandPage() {
  const listings = await getPublicListingsByCategory('import_demand')
  return (
    <>
      <section className="relative overflow-hidden border-b border-gold/10 bg-[#061120] py-14 text-white sm:py-16 lg:py-20">
        ...
        <div className="page-container relative z-10">
          <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.26em] text-gold/72">
            <Link href="/marketplace" className="transition-colors hover:text-gold">Exchange</Link>{' / Import Demand'}
          </p>
          <h1 className="max-w-4xl font-serif text-[2.2rem] leading-[1.06] tracking-normal text-[#f5f1e8] sm:text-5xl lg:text-6xl">
            Import demand and procurement mandates.
          </h1>
          <div className="mt-6 max-w-3xl text-base leading-8 text-white/62 sm:text-lg">
            <p>Active import demand and procurement mandates from licensed buyers, importers and distributors in regulated cannabis markets. Supplier introductions reviewed before any counterparty contact.</p>
          </div>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <Link href="/marketplace/sell" className="btn-marketplace min-h-[52px] justify-center text-center text-sm">Submit supply</Link>
      ...
      <section className="border-b border-gold/10 bg-[#030b16] py-10">
        <div className="page-container grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
          <div key="Medical supply mandates" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Medical supply mandates</h3>
              <p className="text-sm leading-7 text-white/52">Active procurement needs from licensed pharmacies, distributors and medical access programmes seeking compliant supply.</p>
            </div>
          ...
          <div key="Distributor sourcing" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Distributor sourcing</h3>
              <p className="text-sm leading-7 text-white/52">Distribution channel procurement requirements in markets with active licensed-distribution frameworks.</p>
            </div>
          <div key="Wanted by specification" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Wanted by specification</h3>
              <p className="text-sm leading-7 text-white/52">Buyer-specified product, format, potency, certification and origin requirements submitted through Harbourview review.</p>
            </div>
        </div>
      </section>
      <section className="border-b border-gold/10 bg-[#020814] py-8">
        <div className="page-container">
          <div className="rounded-sm border border-gold/20 bg-[linear-gradient(180deg,rgba(10,20,35,0.94)_0%,rgba(5,12,22,0.98)_100%)] p-6">
            <p className="mb-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-gold/72">Reviewed routing only</p>
            <p className="text-sm leading-7 text-white/62">Import demand submissions do not expose buyer identity, procurement volumes, pricing or commercial terms publicly. Harbourview reviews all supplier responses before any introduction is coordinated.</p>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-gold/10 bg-[#020814] py-12 sm:py-16">
        <div className="page-container flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.26em] text-gold/72">Have supply that matches active demand?</p>
            <h2 className="font-serif text-3xl leading-tight tracking-[-0.035em] text-[#f5f1e8] sm:text-4xl">Submit your product for review.</h2>
            <p className="mt-4 text-sm leading-7 text-white/58">GMP-certified cannabis products, export-ready supply and category-specific submissions. Review and qualification before any buyer introduction.</p>
          </div>
