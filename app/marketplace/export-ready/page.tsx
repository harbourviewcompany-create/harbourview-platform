...
export const metadata: Metadata = {
  title: 'Export-Ready Cannabis Products | Harbourview',
  description: 'Reviewed export-ready cannabis product listings from licensed producers. GMP-certified, documented and route-assessed supply for regulated import markets.',
}
...
function ListingCard({ listing }: { listing: PublicListing }) {
  ...
  return (
    <div className="group flex flex-col rounded-sm border border-gold/10 bg-[linear-gradient(180deg,rgba(10,20,35,0.94)_0%,rgba(5,12,22,0.98)_100%)] p-6 shadow-[0_18px_44px_rgba(0,0,0,0.22)] transition-all duration-200 hover:border-gold/30">
      ...
      <Link href={`/contact?ref=${listing.slug ?? listing.id}&type=export-ready`} className="btn-marketplace mt-6 justify-center text-center text-sm">{ctaLabel}</Link>
    </div>
  )
}
...
export default async function ExportreadyPage() {
  const listings = await getPublicListingsByCategory('export_ready')
  return (
    <>
      <section className="relative overflow-hidden border-b border-gold/10 bg-[#061120] py-14 text-white sm:py-16 lg:py-20">
        ...
        <div className="page-container relative z-10">
          <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.26em] text-gold/72">
            <Link href="/marketplace" className="transition-colors hover:text-gold">Exchange</Link>{' / Export-Ready Products'}
          </p>
          <h1 className="max-w-4xl font-serif text-[2.2rem] leading-[1.06] tracking-normal text-[#f5f1e8] sm:text-5xl lg:text-6xl">
            Export-ready cannabis products for regulated import markets.
          </h1>
          <div className="mt-6 max-w-3xl text-base leading-8 text-white/62 sm:text-lg">
            <p>GMP-certified cannabis products from licensed producers with active export capability. Product authority, documentation status and route feasibility reviewed by Harbourview before any importer introduction.</p>
          </div>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <Link href="/marketplace/sell" className="btn-marketplace min-h-[52px] justify-center text-center text-sm">Submit product</Link>
            <Link href="/intelligence" className="btn-intelligence min-h-[52px] justify-center text-center text-sm">Request intelligence</Link>
          </div>
        </div>
      </section>
      <section className="border-b border-gold/10 bg-[#030b16] py-10">
        <div className="page-container grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
          <div key="GMP-certified flower" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">GMP-certified flower</h3>
              <p className="text-sm leading-7 text-white/52">EU-GMP or equivalent certified dried cannabis flower with full batch documentation and CoA availability.</p>
            </div>
          <div key="Extracts and oils" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Extracts and oils</h3>
              <p className="text-sm leading-7 text-white/52">Certified cannabis extracts, oils and derivative products with documented production and quality controls.</p>
            </div>
          <div key="Pharma-grade formats" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Pharma-grade formats</h3>
              <p className="text-sm leading-7 text-white/52">Pharmaceutical-grade cannabis preparations meeting import jurisdiction requirements for capsules, oral solutions and standardised formats.</p>
            </div>
          <div key="Custom and specification" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Custom and specification</h3>
              <p className="text-sm leading-7 text-white/52">Strain-specific, potency-specified and format-customised supply available through reviewed buyer engagement.</p>
            </div>
        </div>
      </section>
      <section className="border-b border-gold/10 bg-[#020814] py-8">
        <div className="page-container">
          <div className="rounded-sm border border-gold/20 bg-[linear-gradient(180deg,rgba(10,20,35,0.94)_0%,rgba(5,12,22,0.98)_100%)] p-6">
            <p className="mb-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-gold/72">Public summaries — no private detail</p>
            <p className="text-sm leading-7 text-white/62">Listings show reviewed public summaries only. Seller identity, pricing, available volumes, documentation packages and route detail are not disclosed without qualification.</p>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-gold/10 bg-[#020814] py-12 sm:py-16">
        <div className="page-container flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.26em] text-gold/72">Export-ready and looking for buyers?</p>
            <h2 className="font-serif text-3xl leading-tight tracking-[-0.035em] text-[#f5f1e8] sm:text-4xl">Submit your product for import routing.</h2>
            <p className="mt-4 text-sm leading-7 text-white/58">GMP-certified cannabis products with active export capability. Harbourview reviews documentation, route feasibility and buyer fit before any introduction.</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <Link href="/marketplace/sell" className="btn-marketplace min-h-[52px] justify-center text-center text-sm">Submit product</Link>
            <Link href="/contact" className="btn-intelligence min-h-[52px] justify-center text-center text-sm">Request a country brief</Link>
          </div>
        </div>
      </section>
    </>
