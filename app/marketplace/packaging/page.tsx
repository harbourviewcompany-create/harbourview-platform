...
export const metadata: Metadata = {
  title: 'Cannabis Packaging & Compliance Materials | Harbourview',
  description: 'Reviewed cannabis packaging, compliance labelling and regulatory-format materials for licensed operators. Harbourview introduction review before counterparty contact.',
}
...
function ListingCard({ listing }: { listing: PublicListing }) {
  ...
  return (
    <div className="group flex flex-col rounded-sm border border-gold/10 bg-[linear-gradient(180deg,rgba(10,20,35,0.94)_0%,rgba(5,12,22,0.98)_100%)] p-6 shadow-[0_18px_44px_rgba(0,0,0,0.22)] transition-all duration-200 hover:border-gold/30">
      ...
      <Link href={`/contact?ref=${listing.slug ?? listing.id}&type=packaging`} className="btn-marketplace mt-6 justify-center text-center text-sm">{ctaLabel}</Link>
    </div>
  )
}
...
export default async function PackagingPage() {
  const listings = await getPublicListingsByCategory('packaging')
  return (
    <>
      <section className="relative overflow-hidden border-b border-gold/10 bg-[#061120] py-14 text-white sm:py-16 lg:py-20">
        ...
        <div className="page-container relative z-10">
          <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.26em] text-gold/72">
            <Link href="/marketplace" className="transition-colors hover:text-gold">Exchange</Link>{' / Packaging'}
          </p>
          <h1 className="max-w-4xl font-serif text-[2.2rem] leading-[1.06] tracking-normal text-[#f5f1e8] sm:text-5xl lg:text-6xl">
            Packaging and compliance materials for licensed cannabis operators.
          </h1>
          <div className="mt-6 max-w-3xl text-base leading-8 text-white/62 sm:text-lg">
            <p>Regulatory-compliant packaging, tamper-evident formats, child-resistant solutions, compliance labelling and co-packaging services for licensed cannabis producers and distributors.</p>
          </div>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <Link href="/marketplace/sell" className="btn-marketplace min-h-[52px] justify-center text-center text-sm">Submit listing</Link>
      ...
      <section className="border-b border-gold/10 bg-[#030b16] py-10">
        <div className="page-container grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
          <div key="Pharmaceutical packaging" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Pharmaceutical packaging</h3>
              <p className="text-sm leading-7 text-white/52">Child-resistant, tamper-evident and pharmacy-format packaging meeting import jurisdiction requirements.</p>
            </div>
          <div key="Compliance labelling" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Compliance labelling</h3>
              <p className="text-sm leading-7 text-white/52">Jurisdiction-specific compliance labelling, batch coding, serialisation and track-and-trace materials.</p>
            </div>
          <div key="Primary packaging" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Primary packaging</h3>
              <p className="text-sm leading-7 text-white/52">Glass, polymer, foil and specialist primary packaging formats for flower, extract, oil and pharma presentations.</p>
            </div>
          <div key="Co-packaging services" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Co-packaging services</h3>
              <p className="text-sm leading-7 text-white/52">Contract packaging, repackaging and compliance-format conversion services for export-ready operators.</p>
            </div>
        </div>
      </section>
      <section className="border-b border-gold/10 bg-[#020814] py-8">
        <div className="page-container">
          <div className="rounded-sm border border-gold/20 bg-[linear-gradient(180deg,rgba(10,20,35,0.94)_0%,rgba(5,12,22,0.98)_100%)] p-6">
            ...
            <p className="text-sm leading-7 text-white/62">Packaging listings show reviewed public summaries. Supplier identity, pricing, MOQ, lead times and compliance documentation are not disclosed without qualification.</p>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-gold/10 bg-[#020814] py-12 sm:py-16">
        <div className="page-container flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.26em] text-gold/72">Packaging supplier or buyer?</p>
            <h2 className="font-serif text-3xl leading-tight tracking-[-0.035em] text-[#f5f1e8] sm:text-4xl">Submit for Harbourview review.</h2>
            <p className="mt-4 text-sm leading-7 text-white/58">Regulatory packaging, compliance materials and co-packaging services for licensed cannabis operators. Review before any buyer or supplier introduction.</p>
          </div>
