...
export const metadata: Metadata = {
  title: 'Cannabis Labs & Testing Services | Harbourview',
  description: 'Reviewed cannabis laboratory and testing services for licensed operators. Potency, contaminants, stability, CoA review and method validation providers.',
}
...
function ListingCard({ listing }: { listing: PublicListing }) {
  ...
  return (
    <div className="group flex flex-col rounded-sm border border-gold/10 bg-[linear-gradient(180deg,rgba(10,20,35,0.94)_0%,rgba(5,12,22,0.98)_100%)] p-6 shadow-[0_18px_44px_rgba(0,0,0,0.22)] transition-all duration-200 hover:border-gold/30">
      ...
      <Link href={`/contact?ref=${listing.slug ?? listing.id}&type=labs-testing`} className="btn-marketplace mt-6 justify-center text-center text-sm">{ctaLabel}</Link>
    </div>
  )
}
...
export default async function LabstestingPage() {
  const listings = await getPublicListingsByCategory('labs_testing')
  return (
    <>
      <section className="relative overflow-hidden border-b border-gold/10 bg-[#061120] py-14 text-white sm:py-16 lg:py-20">
        ...
        <div className="page-container relative z-10">
          <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.26em] text-gold/72">
            <Link href="/marketplace" className="transition-colors hover:text-gold">Exchange</Link>{' / Labs & Testing'}
          </p>
          <h1 className="max-w-4xl font-serif text-[2.2rem] leading-[1.06] tracking-normal text-[#f5f1e8] sm:text-5xl lg:text-6xl">
            Cannabis laboratory and testing services.
          </h1>
          <div className="mt-6 max-w-3xl text-base leading-8 text-white/62 sm:text-lg">
            <p>Potency testing, contaminant screening, stability studies, CoA review services, method validation and laboratory consultancy for regulated cannabis operators and compliance teams.</p>
          </div>
      ...
      <section className="border-b border-gold/10 bg-[#030b16] py-10">
        <div className="page-container grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
          <div key="Potency and cannabinoid testing" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Potency and cannabinoid testing</h3>
              <p className="text-sm leading-7 text-white/52">Validated cannabinoid profiling, potency quantification and certificate of analysis production for regulated markets.</p>
            </div>
          <div key="Contaminant screening" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Contaminant screening</h3>
              <p className="text-sm leading-7 text-white/52">Pesticide residue, heavy metal, mycotoxin, solvent, microbial and foreign material testing services.</p>
            </div>
          <div key="Stability and shelf-life" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">Stability and shelf-life</h3>
              <p className="text-sm leading-7 text-white/52">ICH-guideline stability programmes, shelf-life determination and accelerated degradation studies.</p>
            </div>
          <div key="CoA review and method validation" className="rounded-sm border border-gold/10 bg-[rgba(10,20,35,0.6)] p-6">
              <div className="mb-4 h-px w-10 bg-gradient-to-r from-gold to-gold-light opacity-60" />
              <h3 className="mb-2 text-sm font-semibold text-[#f5f1e8]">CoA review and method validation</h3>
              <p className="text-sm leading-7 text-white/52">Third-party CoA review, method transfer, validation support and laboratory audit readiness services.</p>
            </div>
        </div>
      </section>
      <section className="border-b border-gold/10 bg-[#020814] py-8">
        <div className="page-container">
          <div className="rounded-sm border border-gold/20 bg-[linear-gradient(180deg,rgba(10,20,35,0.94)_0%,rgba(5,12,22,0.98)_100%)] p-6">
            <p className="mb-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-gold/72">Reviewed introductions — no automatic referral</p>
            <p className="text-sm leading-7 text-white/62">Lab and testing service listings show reviewed public summaries. Provider identity, pricing, accreditation detail and service scope are not disclosed without qualification.</p>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-gold/10 bg-[#020814] py-12 sm:py-16">
        <div className="page-container flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.26em] text-gold/72">Lab or testing provider?</p>
            <h2 className="font-serif text-3xl leading-tight tracking-[-0.035em] text-[#f5f1e8] sm:text-4xl">Submit for Harbourview review.</h2>
            <p className="mt-4 text-sm leading-7 text-white/58">Cannabis testing, CoA services, stability studies and laboratory consultancy for regulated operators. Review and qualification before any operator introduction.</p>
          </div>
