...
**Added 2026-08-08 -- read `docs/control/MIGRATION_DRIFT_2026-08-08.md` before diagnosing any "missing table/function/RPC" bug in this repo.** 68 migrations dated 2026-07-01 or later exist in `supabase/migrations` and have never been applied to production. At least five defects that look like application bugs -- clinician verification, the admin review queue, the signal feedback loop, the intelligence graph writer, and the curated signal tier -- are all the same root cause: the code shipped and the schema did not. Checking `supabase_migrations.schema_migrations` first will save hours per incident. There is no automated check for this drift.
