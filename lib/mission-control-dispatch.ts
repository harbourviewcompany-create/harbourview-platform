export type AgentKey = 'ChatGPT' | 'Claude' | 'Perplexity' | 'Gemini'
...
export const providerEnv: Record<AgentKey, { key: string; model: string; label: string }> = {
...
export async function dispatchToAgent(agent: AgentKey, prompt: string): Promise<DispatchResult> {
  ...
  try {
    ...
    return { ok: true, output, blocker: '', provider: provider.label, model }
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown dispatch failure.'
    return { ok: false, output: '', blocker: message, provider: provider.label, model }
  }
}
