...
// catalog. Deliberately NOT the shared api.marketplace_public_listings_v1 --
// that view is the canonical contract for every other /marketplace/* public
// page and must not be extended with supply-specific columns again (see
// migration resolve_supply_view_conflict_decouple_from_marketplace_contract_v2
// for why: two independent sessions extended/reverted that shared view and
// collided). This view shows full real data (stock, MOQ, lead time,
// brand/model, real compliance flags) per explicit product decision --
// contrast with api.supply_catalog_public_v1, which redacts the same
// fields and predates that decision.
...
function baseParams(limit = 300): URLSearchParams {
  // No sold_by_harbourview filter needed here -- the view itself is already
  // scoped to Harbourview-direct, approved, publicly-visible listings only.
