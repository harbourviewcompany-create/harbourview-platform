...
import { GLOBE_GROUP_ROTATION } from './globe-plate-config'
...
/**
 * Apply the globe group rotation to convert a local-space position/direction
 * into world space. Must stay in sync with GLOBE_GROUP_ROTATION.
 */
function applyGlobeGroupRotation(x: number, y: number, z: number): [number, number, number] {
  const { rx, ry } = GLOBE_GROUP_ROTATION
