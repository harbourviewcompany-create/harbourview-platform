...
function buildCosts(plan: Awaited<ReturnType<typeof deriveCorridorPlan>>): CostEstimate {
  ...
  const components: CostComponent[] = plan.estimatedCostRanges.map((entry) => {
    ...
    return {
      ...
      basis: entry.range ? 'Editorial jurisdiction playbook range; not a landed-cost quotation.' : 'No structured cost range published.',
      evidenceIds: [],
      confidence: 'partial',
    }
  })
...
function buildDecision(input: MarketEntryInput, plan: NonNullable<Awaited<ReturnType<typeof deriveCorridorPlan>>>, evidence: Awaited<ReturnType<typeof buildEvidenceSnapshot>>, execution: ExecutionGraph): CorridorDecisionRecord {
  ...
  const originAllowed = jurisdictionRoleAllows(input.originIso2, 'origin')
  const destinationAllowed = jurisdictionRoleAllows(input.destinationIso2, 'destination')
  const originGate: ReadinessGate = { id: 'origin-role', label: 'Origin trade role', state: originAllowed ? 'pass' : 'fail', reason: originAllowed ? 'Jurisdiction is configured as an origin or both.' : 'Jurisdiction is not configured as an eligible origin in the current trade matrix.', evidenceIds: [] }
  const destinationGate: ReadinessGate = { id: 'destination-role', label: 'Destination trade role', state: destinationAllowed ? 'pass' : 'fail', reason: destinationAllowed ? 'Jurisdiction is configured as a destination or both.' : 'Jurisdiction is not configured as an eligible destination in the current trade matrix.', evidenceIds: [] }
  const productSupported = Boolean(plan.corridorRef) || input.productClass === 'any'
  const productGate: ReadinessGate = { id: 'product-compatibility', label: 'Product compatibility', state: productSupported ? 'pass' : 'fail', reason: productSupported ? 'The selected product is represented by the corridor model.' : 'The selected product is not represented by the tracked corridor product matrix.', evidenceIds: [] }
  const evidenceStatuses = evidence.references.map((ref) => ref.status)
  const allVerified = evidenceStatuses.length > 0 && evidenceStatuses.every((status) => status === 'verified')
  const evidenceBlocked = evidenceStatuses.some((status) => status === 'stale' || status === 'unverified' || status === 'conflicted')
  const evidenceGate: ReadinessGate = {
    id: 'primary-source-evidence',
    label: 'Claim-level primary-source evidence',
    state: allVerified ? 'pass' : evidenceBlocked ? 'fail' : 'warn',
    reason: allVerified
      ? 'Both corridor jurisdictions have current claim-level primary-source evidence with effective dates and immutable source hashes.'
      : 'The corridor does not have complete current claim-level primary-source evidence with immutable source hashes; it cannot be treated as executable.',
    evidenceIds,
  }
  const cycleGate: ReadinessGate = { id: 'execution-graph', label: 'Execution dependency graph', state: execution.cycleDetected ? 'fail' : 'pass', reason: execution.cycleDetected ? 'A dependency cycle was detected.' : 'Execution tasks form an acyclic dependency graph.', evidenceIds: [] }
...
export async function buildMarketEntryPlan(rawInput: MarketEntryInput, options: { now?: Date; freshnessDays?: number } = {}): Promise<MarketEntryPlan | null> {
  ...
  const input: MarketEntryInput = { originIso2, destinationIso2, productClass, requestedAt: rawInput.requestedAt }
  const plan = await deriveCorridorPlan(originIso2, destinationIso2, { productClass })
  if (!plan) return null
  const evidence = await buildEvidenceSnapshot(plan, { ...options, productClass })
  ...
  const reproducibilityKey = createHash('sha256').update(JSON.stringify({ input, evidence: evidence.snapshotId, planTrust: plan.trust, execution })).digest('hex')
  return {
    planVersion: 2,
    planId: randomUUID(),
    input,
    ...
    evidence,
    execution,
    documentation: { required: plan.documentationChecklist.filter((item) => item.required).map((item) => item.label), optional: plan.documentationChecklist.filter((item) => !item.required).map((item) => item.label), orientationOnly: !decision.executable },
    costs,
    notes: plan.notes,
...
export function getCoverageStats() {
  ...
  return { jurisdictions: APPLICABLE_TRADE_JURISDICTIONS.length, origins: origins.length, destinations: destinations.length, directedPairs: origins.reduce((count, origin) => count + destinations.filter((destination) => destination.iso2 !== origin.iso2).length, 0), products: SUPPORTED_PRODUCTS }
}
