 ...
 *
 * This layer composes the existing governed evidence, formulary, patient,
 * consent, professional-verification and RLS contracts. It is not a second
 * evidence store and it does not make autonomous treatment recommendations.
 */

export type ClinicalScalar = string | number | boolean | null
export type ClinicalStructuredValue = ClinicalScalar | ClinicalStructuredObject | ClinicalStructuredValue[]
export type ClinicalStructuredObject = { [key: string]: ClinicalStructuredValue }
...
export type ClinicalSafetyRule = {
  ...
  subject: string
  appliesWhen: ClinicalStructuredObject
...
export type MedicinalProduct = {
  ...
  composition: { [key: string]: string | number | null }
 ...
 * document or record. Homepages, generic indexes and search portals fail closed.
 */
export function isInspectableClinicalSource(url: string | null | undefined): boolean {
  ...
  try {
    ...
    if (GENERIC_SOURCE_URLS.has(parsed.toString())) return false
    if (parsed.pathname === '/' && !parsed.search) return false
    if (SEARCH_PORTAL_PATHS.some((pattern) => pattern.test(parsed.pathname))) return false
    if (parsed.hostname === 'pubmed.ncbi.nlm.nih.gov' && !/^\/\d+\/?$/.test(parsed.pathname)) return false
    return true
  } catch {
    return false
  }
...
export function buildAskClinicalResponse(question: string, records: ClinicalEvidenceRecordDTO[]): AskClinicalResponse {
  ...
  const stale = eligible.filter((record) =>
    ['stale', 'review-required', 'source-degraded'].includes(record.freshnessStatus ?? 'current'),
  )
  ...
  return {
    ...
    question,
    answer: summaries.join(' ') || 'Governed records matched, but no reviewed summary is available.',
...
export function derivePrescriberReadiness(input: {
  ...
  const dimensions: ReadinessDimension[] = [
    {
      ...
      explanation: input.hasClinicalContext
        ? 'Clinical context is available.'
        : 'Condition, population and treatment objective are incomplete.',
    },
    {
      key: 'evidence',
      label: 'Evidence',
      state: input.hasInspectableEvidence ? 'ready' : 'unknown',
      explanation: input.hasInspectableEvidence
        ? 'Governed, inspectable evidence is available.'
        : 'No prescriber-inspectable evidence is available for this context.',
    },
    {
      key: 'authority',
      label: 'Professional authority',
      state: !input.authorityKnown ? 'unknown' : input.authorityAllowsAction ? 'ready' : 'blocked',
      explanation: !input.authorityKnown
        ? 'Professional authority has not been resolved.'
        : input.authorityAllowsAction
          ? 'Current authority contract allows the requested action.'
          : 'Current authority contract does not allow the requested action.',
    },
    {
      key: 'product',
      label: 'Product / formulation',
      state: input.productResolved ? 'ready' : 'unknown',
      explanation: input.productResolved
        ? 'A governed product/formulation record is resolved.'
        : 'No governed product/formulation has been resolved.',
    },
    {
      key: 'safety',
      label: 'Safety',
      ...
      explanation:
        input.unresolvedMajorSafetyItems > 0
    ...
    },
    {
      ...
      explanation: input.monitoringDefined
        ? 'Monitoring and stopping criteria are defined.'
        : 'Monitoring and stopping criteria are not yet defined.',
    },
    {
      key: 'consent',
      label: 'Consent',
      state: input.consentConfirmed ? 'ready' : 'blocked',
      explanation: input.consentConfirmed
        ? 'Required consent is recorded.'
        : 'Required clinical consent has not been confirmed.',
    },
  ]
  ...
  const allReadyOrAttention = dimensions.every(
    (dimension) => dimension.state === 'ready' || dimension.state === 'attention',
  )
