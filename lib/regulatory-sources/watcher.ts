...
function extractApiItems(json: unknown, source: WatchableRegulatorySource, fallbackUrl: string): ExtractedSourceItem[] {
  const records = Array.isArray((json as any)?.results)
    ? (json as any).results
  ...
  return records.slice(0, 20).map((record: any, index: number) => {
    ...
    const published = attributes?.publication_date || attributes?.postedDate || attributes?.lastModifiedDate || null
    const text = normalizeWhitespace(`${title} ${summary}`)
