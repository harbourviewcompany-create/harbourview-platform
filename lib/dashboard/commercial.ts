...
function fallbackMarketplace(countryName: string): CountryMarketplaceProfile {
  return {
    ...
    listings: [
      { id: 'consumables-packaging', title: `${countryName} consumables and packaging requests`, category: 'Consumables', locationLabel: countryName, availabilityLabel: 'Quote-ready categories', ctaLabel: 'Request quote', href: '/marketplace/quote' },
      { id: 'equipment-used-surplus', title: 'Used and surplus processing equipment', category: 'Used equipment', locationLabel: 'Regional availability', availabilityLabel: 'Reviewed seller route', ctaLabel: 'View equipment', href: '/marketplace/used-surplus' },
      { id: 'lawful-trade-review', title: 'Lawful import/export opportunity review', category: 'Cannabis products / import-export opportunities', locationLabel: countryName, availabilityLabel: 'Review-gated', ctaLabel: 'Request review', href: '/compliance/request-support' },
    ],
    wantedRequests: [
...
export function assertCountryDashboardPublicSafe(value: unknown) {
  ...
  function visit(input: unknown) {
    ...
    for (const [key, child] of Object.entries(input as Record<string, unknown>)) {
      if (FORBIDDEN_COUNTRY_DASHBOARD_PUBLIC_KEYS.some((forbidden) => forbidden.toLowerCase() === key.toLowerCase())) hits.push(key)
      visit(child)
    }
  }
  visit(value)
