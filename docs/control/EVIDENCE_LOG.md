...
**Scope:** repository-only across five areas — public-route render strategy, CI trigger topology, the `npm test` gate, `CommandCentre` bundle composition, and one migration file. One new migration is added but **not applied**; no production change was made in this session. All production database access was read-only.
...
**5. Database, measured before acting.** `source_registry` holds 1,826 rows in 3,400 kB of which 2,040 kB is index, across 17 indexes, and had 349,242 sequential scans — the highest of any table. Five of those indexes cover `source_url` or a normalisation of it; only `source_registry_source_url_uniq` is backed by a UNIQUE constraint (confirmed via `pg_constraint contype='u'`). Migration `20260820190000` drops the two plain duplicates, reclaiming ~456 kB and two index writes per registry upsert, weakening no uniqueness guarantee. The two normalized-url indexes are deliberately retained: unused by scan count, but encoding two different normalisation rules, so dropping them changes de-duplication semantics.
...
- `npm run build` — exit 0, 433 routes; all 21 converted routes confirmed in the build report as prerendered with their revalidate windows.
...
**Decision:** **GO for the repository change.** No production action taken; migration `20260820190000` is unapplied and needs sign-off, as does any move on the `anon` write grants. Opened as a draft PR rather than merged — the ISR freshness windows and the test quarantine both warrant review before this lands.
