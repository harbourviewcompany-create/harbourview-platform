...
**Evidence ID:** `HV-CLINICAL-PUBLICATION-GATE-20260819`

**Scope:** repository-only. Adds `20260819170000_clinical_reviewer_credential_integrity.sql`, two admin API routes, one shared governance module, one test suite and `docs/control/CLINICAL_PUBLICATION_GATE_20260819.md`. **No migration applied, no credential created, no review recorded, no record published.**

**Diagnosis (verified live, read-only, `zvxdgdkukjrrwamdpqrg`).** The clinical corpus is written and invisible. `clinical_evidence_records` holds 23 rows: **3 published** (the original Canadian regulatory documents, no condition label) and **20 under-review** — every condition-level record. `search_clinical_evidence_records` filters on `published`, so `search_clinical_evidence_records('Dravet syndrome','CA',20)` returns **0**. Supporting tables `clinical_reviewer_credentials`, `clinical_grade_assessments`, `clinical_evidence_snapshots`, `clinical_outcome_evidence` and `clinical_condition_terms` are all **empty**.

`trg_clinical_require_publication_review` requires an approved `provenance` review always, plus an approved credential-bound `clinician`/`pharmacist` review whenever certainty is graded. All 20 records carry graded certainty. Simulating the trigger's own predicates as SELECTs across all 20: **0 would publish** — 0 have provenance, 20 need clinical review, 0 have it. The admin console writes via the service-role client, which bypasses RLS but not triggers, so it cannot route around the gate.

**Missing surface (the actual blocker).** `/api/clinical/verification/request` calls `clinical_request_verification` and `/api/clinical/admin/verify` calls `clinical_admin_verify_professional`; **neither RPC exists in production** — the `AGENT_OPERATING_FACTS.md` pattern again. Neither writes to `clinical_reviewer_credentials` in any case, and the admin review console only flips `review_status`. There was therefore **no application path to any of the three gates**, leaving raw production SQL as the only route to a record whose entire purpose is auditability.

**Ordering correction.** An earlier note in this session recommended pre-populating GRADE assessments "so the reviewer has something to review". That is structurally impossible: `clinical_grade_assessments.review_id` is `NOT NULL` and references `clinical_evidence_reviews`, so the assessment cannot precede its review. Recorded in the control document rather than quietly dropped.

**Integrity gaps closed.** `clinical_reviewer_credentials.user_id` and `verified_by_user_id` had no foreign keys (any UUID accepted, including one belonging to no account); a credential could be marked `verified` with no record of who checked the register, and an admin could verify their own credential while holding the same role that authorises the write; `valid_from` could postdate `valid_until`; and every `clinical_grade_assessments` GRADE domain was unconstrained text, so `"very serious"` would persist silently and never match the vocabulary the application reads. Both tables are empty in production, so all constraints are added without backfill. **No existing gate is relaxed.**

**Verification.** `npm run typecheck` clean. `npm run build` clean; both new routes appear in the route manifest. `npx vitest run tests/clinical/` — 16 new tests pass; 8 failures across 4 files reproduce identically on `origin/main` with the diff stashed (pre-existing, untouched by this change). `npm run lint` — **207 problems / 11 errors, byte-identical to `origin/main` with the diff stashed**; all 11 errors are pre-existing `no-explicit-any` in `lib/clinical/productionQuery.ts` and `lib/clinical/sourceCurrentness*.ts`, none in changed files. `node scripts/check-migration-sql-parses.mjs` → GO, 969 migrations parse. `node scripts/check-migration-filenames.mjs` fails on a pre-existing duplicate prefix `20260813010000` (two unrelated files on main); `20260819170000` is unique.

**Migration proven against a scratch cluster, never production.** A local PostgreSQL 16 instance was initialised with a fixture reproducing the live constraints and triggers, the migration applied, then eight cases exercised: credential for a non-account `user_id` → FK violation; `verified` with no verifier → `..._verifier_recorded`; self-verification → `..._no_self_verification`; inverted validity window → `..._validity_window`; verification by a second admin → accepted; GRADE domain `"very serious"` → `..._downgrade_domains_check`; `publication_bias = 'very-serious'` → `..._publication_bias_check`; valid assessment → accepted. Final state 1 credential, 1 assessment. Cluster destroyed afterwards.

**Branch note.** `claude/clinical-section-improvements-q251k3` was reset onto `origin/main` (`fa23a8a`). Its previous head `15c7955` is preserved in closed PR #1473, which was closed as superseded by #1479 and #1509–#1527; replaying it would have regressed the multi-country authority registry now on main.

**Not closed.** Four decisions remain, none technical: who the first reviewer is, which register verifies them and who performs the check, what that reviewer attests to given `clinical_grade_assessments` is empty, and the scope of the first release. The sourcing-bar question raised on 2026-08-16 — primary sources only versus secondary synthesis with attribution — is still unanswered and still blocks content work.

**Decision:** **GO for the repository change.** Applying `20260819170000` to production is gated on owner sign-off under Rule 3c. No credential, review or publication is authorised by this entry.
