...
- `npm run build` — exit 0, 433 routes. The 15 non-parameterised converted routes report as `○` static with their windows. An earlier version of this entry claimed all 21 did; that was wrong. Exporting `revalidate` on a dynamic segment does **not** opt it into ISR on its own — `/intelligence/playbooks/[country]`, `/marketplace/consumables/[id]`, `/marketplace/listings/[slug]`, `/professionals/[slug]` and `/supplier-directory/[id]` still reported `ƒ` (server-rendered every request). They now export `generateStaticParams()` returning no paths and report `●`; verified at runtime against `next start`, where `/marketplace/consumables/123`, `/marketplace/listings/[slug]` and `/supplier-directory/42` each go `x-nextjs-cache: MISS` then `HIT` with `s-maxage=1800`/`3600` matching their windows. The playbooks and professionals routes could not be observed locally — middleware redirects them to `/login` when Supabase env vars are absent — but carry identical configuration. `/marketplace/genetics/[slug]` is deliberately left `force-dynamic`: its query path uses the cookie-bound `createClient()`, so no window would apply. Caught in review by Codex on PR #1605.
