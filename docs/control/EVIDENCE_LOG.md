...
|---|---|---|---|---|---|
| 2026-09-16 | Repointed code references to migrations renamed by #1823 — unblocks `npm run test` and the `decision-intel-stage0` verification workflow | Repo-wide scan for `supabase/migrations/<14-digit>_<name>.sql` across `*.ts,tsx,mjs,js,yml,yaml,sh,json`; per-pair byte-identity check of pre-deletion blob vs current file (`git show <deletion-commit>^:<old path>` piped to `sha256sum`); `npx vitest run` on each affected suite before and after; `python3 -c "yaml.safe_load(...)"` on the edited workflow; `npm run lint`; `npm run typecheck`; `npm run test`; `npm run test:security`; `npm run build`; `git diff --check` | PASS. #1823 (`fix(governance): reconcile production drift and harden CI controls`) renamed migration files without updating code that reads them by path, leaving `ENOENT` failures on pristine `main`. Scan found **126** distinct migration path references in functional files, **16** unresolvable. Nine of those (`00000000000000_*`) are false positives — workflow heredocs (`cat > supabase/migrations/00000000000000_*.sql <<'SQL'`) that **create** the file at runtime rather than read it; left untouched. Of the remaining seven, **five mappings were repointed here across every file that referenced them — 9 files, 22 path strings** — after confirming each rename is byte-identical, so no assertion is evaluated against different SQL: `20260802080000`→`20260912103805` (harden_eval_labels_and_alert_delivery), `20260810202000`→`20260911225151` (decision_intel_stage0_completion_hardening), `20260810222500`→`20260912103836` (harden_edge_function_cron_auth), `20260808190000`→`20260911225008` and `20260808203000`→`20260911225106` (decision_intel_stage0 first_slice / review_fixes). Suite moves from **5 failed / 149 passed / 1200 tests** to **2 failed / 152 passed / 1209 tests**; the three repointed suites go 19/19 and `tests/intel/decisionIntelFirstSlice.test.ts` stays 23/23. **A first pass of this fix was incomplete and the re-scan caught it:** the scan was repo-wide but the edits were applied only to the four files identified during the original triage, leaving stale paths in `decision-intel-completion-hardening-verify.yml`, `decision-intel-first-slice-verify.yml`, `pr1224-pipeline-migration-dry-run.yml` and `tests/intel/decisionIntelFirstSlice.test.ts`. CI surfaced it as a `Publication, withdrawal, routing and history contract` failure on #1920. The mappings were then applied globally to every matching file. **A second gap was then found the same way:** the scan's `--include` list covered `ts,tsx,mjs,js,yml,yaml,sh,json` but **not `.sql`**, and `tests/sql/pr1224_pipeline_hardening_dry_run.sql:80` does `\i supabase/migrations/20260802080000_harden_eval_labels_and_alert_delivery.sql`. That surfaced as a `postgres-17` failure on #1920 — a check that had been dormant because its own trigger paths pointed at a file that no longer existed, and which only started running once this PR edited the workflow. Reproduced locally against PostgreSQL 16.13 (`initdb` + `pg_ctl` + `psql -v ON_ERROR_STOP=1 -f tests/sql/pr1224_pipeline_hardening_dry_run.sql`): failed at line 80 with the identical `No such file or directory`, exit 3; after repointing, the same fixture runs to completion on a clean database, exit 0, with `hv_alert_tick` returning `{"ok": true, ...}` and both trailing assertion `DO` blocks executing. The scan was then re-run with **no extension filter at all**, confirming the only unresolvable references left are the runtime-created heredoc paths, the two deliberate exclusions below, and two GitHub URLs inside string literals in `20260710084232_batch_10_hardening_overlay_controlref.sql` and `20260710235525_batch_10_optimization_controlref_log.sql` — historical provenance records, deliberately untouched because editing a migration file would break its content-hash binding in the decisions ledger. lint 0 errors / 213 warnings (pre-existing baseline), typecheck exit 0, `test:security` 123/123 across 21 files, build exit 0, edited workflow re-parses with 11 trigger paths and both migration paths now resolving. **Two pre-existing failures deliberately NOT addressed — different root cause, not migration renames:** `tests/harbourview/monetization-lifecycle-contract.test.ts` (three content assertions against `app/api/stripe/webhook/route.ts` for strings the route no longer contains — billing logic, needs its own review) and `tests/ci/workflow-infrastructure-hygiene.test.ts` (`expected -1 to be greater than -1`, an ordering lookup). **Two of the seven broken references deliberately NOT repointed, both needing a decision rather than a mechanical fix:** `20260818224000_clinical_evidence_spine_reconcile` → `20260819100621_...` is **not** byte-identical, so repointing would silently change what the consumer asserts against; and `20260820180000_p0_p2_pipeline_optimization.sql` has **no** replacement file and no git history in this repository — its two consumers (`.github/workflows/apply-p0-p2-canonical-pipeline.yml`, guarded by `if [ -f ... ]`, and `tests/supabase/p0p2CanonicalFinish.test.ts`) currently tolerate its absence. **Also unresolved, same root cause:** `supabase/release-controls/pending-production-migration-decisions.json` still keys on the pre-rename versions, which is why `check-pending-production-migration-decisions.mjs` reports five `decision file is absent` entries and `governance-gate` stays red on pristine `main`; that ledger is content-hash-bound and reconciling it is a governance call. The comments in `tests/security/edge-function-auth-hardening.test.ts` that cite `20260810222500` were **left unchanged on purpose** — they describe that ledger's binding, which still uses the old version, so rewriting them would have made them false. **Why this mattered beyond any one PR:** `AGENTS.md` requires every PR to add a `docs/control/EVIDENCE_LOG.md` row, and that file is a trigger path for at least four verification workflows (`decision-intel-stage0-review-fixes-verify`, `decision-intel-completion-hardening-verify`, `regional-routing-verification`, `elite-digest-boundary-hardening-verification`, `elite-digest-forward-repair-verification`); the first ran `psql` against a renamed file and the other three run `npm run test`, so every compliant PR failed checks it did not cause. No production schema, RLS, auth, billing behaviour, migration content, or canonical copy changed — this PR repoints twenty-two path strings across five version mappings in nine files, plus this evidence row. Generated globe timestamp restored and absent from the diff | PR #1920; branch `claude/fix-renamed-migration-path-references`; base `49f61fa`; root cause of failures triaged on #1908 and #1912 | Current — verified locally; two references and the decisions ledger deliberately left for a decision |
