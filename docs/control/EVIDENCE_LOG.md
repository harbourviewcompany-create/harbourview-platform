...
|---|---|---|---|---|---|
| 2026-09-16 | `STRIPE_PRICE_MARKET_ENTRY_REPORT` registered in the environment manifest — closes one of three `check:env-manifest` gaps failing `Security / Leakage` on `main` | `node scripts/check-environment-manifest.mjs` (before and after); `npm ci`; `npm run lint`; `npm run typecheck`; `npm run test` (on the change and again on stashed clean `main` for attribution); `npm run build`; `python3 -m json.tool` on the edited manifest | **PARTIAL PASS — intended scope complete, gate still red by design.** `main` @ `36dc34d` fails `check:env-manifest` on three unregistered references: `OPENAI_KEY` (`.github/workflows/pr-agent.yml`), `PYTHONUNBUFFERED` (two governance-audit workflows), and `STRIPE_PRICE_MARKET_ENTRY_REPORT` (`app/api/market-entry/checkout/route.ts`, shipped in #1895). Only the Stripe entry is fixed here, deliberately: the other two are not the same kind of defect and were left for an explicit decision. `PYTHONUNBUFFERED: '1'` is a hardcoded literal, not a provisionable secret — the checker flags any `env:` key without distinguishing secret references from inline constants, so the correct fix is to the checker or the workflow manifest, not a blanket manifest entry. `OPENAI_KEY` is a live third-party secret whose only consumer, the `AI PR review` job, was observed **cancelled** on all three open PRs checked (#1906, #1908, #1909); registering it would quietly legitimize a credential that may be vestigial. Entry placed in `config/environment-application-manifest.json` rather than the parent manifest because the parent is an audited snapshot (`auditedAt: 2026-08-15`, `sourceTreeSha` pinned) and the application manifest exists precisely for post-audit additions; classification mirrors the existing `stripe-live-price-ids` group (Runtime only / Plain variable / provider `stripe` / `next-app` / live). After the change `check:env-manifest` no longer reports the Stripe variable; it still exits 1 on the remaining two, so `Security / Leakage` stays red until those are decided — **this PR does not turn that gate green and does not claim to**. QA: lint 0 errors / 212 warnings (pre-existing baseline); typecheck exit 0; build exit 0; Vitest **5 failed / 1200 passed / 15 todo across 156 files** — byte-identical to the same suite run on stashed clean `main`, so zero regressions. **Separately discovered during verification and not fixed here:** three of those five pre-existing failures are `ENOENT` on migration files that were renamed out from under code still referencing them by path — `20260810222500_harden_edge_function_cron_auth.sql` (`tests/security/edge-function-auth-hardening`), `20260802080000_harden_eval_labels_and_alert_delivery.sql` (`tests/harbourview/monetization-lifecycle-contract`), and `20260810202000_decision_intel_stage0_completion_hardening.sql` (`tests/intel/decisionIntelJurisdictionNavigation`). All three versions are in the same set of eight that also broke `supabase/release-controls/committed-not-applied-baseline.json`, which PR #1908 repairs — confirming the rename fallout is broader than the baseline file alone and that #1908 closes only part of it. `20260810222500` has prior history in this log: the 2026-08-13 row records it failing `check-pending-production-migration-decisions.mjs` on a blob-hash mismatch. No production schema, RLS, auth, billing behaviour, or canonical copy changed — this is a manifest registration only. Provisioning the variable in Vercel was **not** verified and remains outstanding; until it is set, `/api/market-entry/checkout` fails closed with HTTP 503 | PR #1912; branch `claude/review-updates-g04obm`; base `36dc34d` | Current — verified locally; env-manifest gate intentionally still red on two undecided variables |
| 2026-09-16 | Command overview implementation — per-user since-last-visit delta, role-relevant attention slots, honest row freshness, watch-rule block (`docs/COMMAND_SURFACE_SPEC.md` 4.1-4.4) | `npm run lint`; `npm run typecheck`; `npm run test`; `npm run test:quarantined`; `npm run test:mobile-intel`; `npm run test:command-centre-production`; `npx vitest run tests/dashboard/commandDelta.test.ts`; `env -u SENTRY_AUTH_TOKEN npm run build`; baseline comparison runs of lint and `test:quarantined` against `origin/main` | PASS. lint 0 errors / **213 warnings, identical to `origin/main` measured the same way** — no new warning. typecheck exit 0. Default suite 1,215 passed / 15 todo with the same 6 pre-existing failures as the baseline (1,199 + 16 new `commandDelta` tests). mobile-intel 7/7; command-centre-production 22/22; build exit 0, 150/150 static pages. `test:quarantined` 5 passed / 6 failed — **exact parity with `origin/main`**, verified by running it on both. **Two regressions this change caused were found by that comparison and fixed, not waived:** (1) `tests/dashboard/mobileCommandCentreV2.test.ts` pinned the Attention pulse at 5, which is 3 once the two corridor launchers stop counting as attention — the assertion was updated to 3 *and* a new case added asserting the launchers still render as tools and never appear in the priority rows; (2) `tests/harbourview/p0-identity-org-context.test.ts` pinned the exact `.select('country_iso2, role_id, active_workspace_id')` source literal, which a new column breaks — rewritten to assert those three columns are present rather than the literal string, preserving the guard. Both suites are in the `npm test` quarantine list but are named directly by workflows, so both run in CI. Build-generated `data/globe/natural-earth-countries.ts` timestamp restored and is absent from the diff. **No production database change**: the migration is committed only and is NOT applied — `AGENT_OPERATING_FACTS.md` 1. No RLS, auth, billing, DTO or public-copy change | PR #1930; branch `claude/command-improvements-qfsxgn` | Current — code merged-ready pending review; migration committed but **unapplied and requiring Tyler's Rule 3c sign-off** |
| 2026-09-16 | Command overview audit — spec proposing per-user "since last visit" state, role-relevant attention items, real row metadata and watch-rule wiring; plus a verified upstream signal-supply defect | Docs-only, no command. Live read-only verification via `mcp supabase execute_sql` against `zvxdgdkukjrrwamdpqrg` (`information_schema.columns`, `public.signals`, `api.signals_with_quality`, `public.cc_watch_rules`, `public.user_dashboard_preferences`) | Added `docs/COMMAND_SURFACE_SPEC.md`. Three findings worth recording: (1) `public.signals.role_families` is populated on **0 of 14,169 rows** — `lib/signals/routing.ts` exists and has never run, so role selection cannot change Command's content; (2) the two "Requires attention" items are hardcoded in `useMobileCommandModel.ts:120-151`, prepended to `nextActions` with `tone:'gold'`, and therefore occupy both slots for every user regardless of role — a Doctor/Prescriber is shown "Run landed cost + sensitivity"; (3) **supply, not design, is the binding constraint on the feed** — 12 reviewed Canadian signals in the last 7 days resolve to 2 distinct URLs on 1 identical timestamp, and globally 196 reviewed rows across 41 countries carry 86 URLs and only 12 distinct timestamps, so freshness reflects pipeline run time rather than events. No code, schema, or production state changed by this PR | PR pending; branch `claude/command-improvements-qfsxgn` | Current — spec proposed, not implemented; schema decision in §6 open for Tyler |
...
| 2026-08-22 | Global Reg OS Phase 0 gate no longer triggers on `EVIDENCE_LOG.md` — removing a deadlock that made migration PRs unlandable | `python3 -c "yaml.safe_load(...)"` on the edited workflow; `node --test tests/scripts/migration-ledger-manifest.test.mjs`; trigger-path reading cross-checked against the empirical result recorded in the 2026-08-13 row below | PASS — `AGENTS.md` requires an `EVIDENCE_LOG.md` entry from **every** merged PR, and `PR_REVIEW_CHECKLIST.md`'s baseline gate says to reject a PR that lacks one. `global-reg-os-phase0-replacement.yml` listed `docs/control/EVIDENCE_LOG.md` as a trigger path and its `Verify migration isolation` step asserts `git diff --exit-code <base.sha> HEAD -- supabase/migrations`. Those two rules are mutually unsatisfiable for any PR that touches a migration: record the required evidence and the gate fires and fails; omit it and the baseline gate rejects the PR. **This is not a new finding — the 2026-08-13 row below hit the same wall** and resolved it by splitting the evidence row into a separate docs-only PR, noting it preferred that to "editing the control plane to work around a content rule", with the trigger confirmed empirically on PR #1387. That workaround is reversed here, deliberately: splitting does not actually satisfy the baseline gate (the migration PR still merges with no evidence row of its own), and it taxes every future migration PR. The defect is the trigger's **scope**, not either rule's substance — `EVIDENCE_LOG.md` is written by every PR in the repository, so listing it made a Global Regulatory OS gate fire on work with no Global Regulatory OS involvement. No assertion in the workflow was changed or weakened; the remaining four paths still cover its actual surface, and a PR touching both the evidence log and that surface still trips it. YAML re-parsed clean, four paths remain; ledger parser tests 14/14 pass | Branch `fix/phase0-gate-scope`, base `04012b49`. **Deliberately split from the collision fix it unblocks** — that change touches `supabase/migrations`, and this PR must land first for it to be landable at all. Scope here is one workflow's `paths:` list and nothing else | Current |
