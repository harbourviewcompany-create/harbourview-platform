...
|---|---|---|---|---|---|
| 2026-09-17 | Repoint 9 files at the post-rename migration filenames — #1823 renamed 8 migrations but left workflows and tests reading the pre-rename paths, so three suites could not load and three workflows died on `psql: No such file or directory` | `npx vitest run` on the four affected suites; `npm run test` (twice, on this branch); `npm run test` on `origin/main` for baseline; `npm run typecheck`; `npm run lint`; `env -u SENTRY_AUTH_TOKEN npm run build`; every referenced `supabase/migrations/*.sql` path resolved against the working tree | PASS. Four previously-ENOENT suites now load: **42/42 pass**. Full suite `origin/main` **6 failed / 1,199 passed / 1,220 total across 6 failing files** -> this branch **5 failed / 1,208 passed / 1,228 total across 3 failing files**: 8 tests became runnable and 3 whole suites recovered. The 3 remaining failures (workflow-infrastructure-hygiene, design-tokens, monetization-lifecycle-contract) are unrelated to migration paths and pre-exist on `main`. A 4th name (`marketplace-capture-route`) appeared on the first full run only, passed in isolation and on re-run — flaky under parallel load, untouched by this diff. typecheck exit 0; lint 0 errors / **214 warnings, identical to `origin/main`**; build exit 0, 150/150 static pages. **Scope deliberately excluded**: all of `supabase/release-controls/**` and `scripts/check-pending-production-migration-decisions.mjs` (governed, content-hash-bound, actively being reconciled on `main` by #1933/#1936); `tests/scripts/migration-ledger-manifest.test.mjs` (uses those version strings as synthetic fixtures, not real paths); `tests/scripts/pending-production-migration-decisions.test.mjs` (asserts against the governed ledger and must change with it). **Near-miss recorded**: a missing-path sweep flagged `20260820180000_p0_p2_pipeline_optimization.sql`, which is *deliberately* absent — `apply-p0-p2-canonical-pipeline.yml` declares it `FORBIDDEN_VERSION` and fails if the file exists. Not touched. No schema, RLS, auth, billing, DTO or public-copy change; no migration content altered | Branch `fix/stale-migration-path-references-20260917`; base `34032df` | Current |
