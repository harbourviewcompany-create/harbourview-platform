...
|---|---|---|---|---|---|
| 2026-09-02 | Migration drift reconciliation: recovered the 4 production versions claimed by no open PR, identified the drift generator, and retracted the "applied SQL is not recoverable" claim | Read-only MCP `execute_sql` against `supabase_migrations.schema_migrations` (`statements`, `created_by`, `md5(statements[1])`) and `pg_proc.proconfig`; GitHub Actions run 33667689428 job 100373360150; `git ls-tree`/`git show` against PR #1740's head; local md5 comparison of every written file against its live statement. Full QA suite run (correcting an earlier claim in this same row that it could not be): `node scripts/check-migration-sql-parses.mjs --self-test` and the full parse run; `npm ci`; `npm run typecheck`; `npm run lint`; `npm run test`; `npm run build`. `NPM_INSTALL_ENVIRONMENT_HOLD.md` was cited to skip the gate and is **stale** — `npm ci` completes here in ~20s; corrected under PR #1747 | PASS. **Migration SQL parse gate: 1,010 migrations parsed, `GO: every migration parses as valid PostgreSQL`, self-test 9/9** — this is the real gate for the four added files and it passes. typecheck exit 0; lint 0 errors / 209 warnings (unchanged baseline); 1,169 tests passed across 142 files + 2 skipped, 15 todo; production build exit 0, 151/151 static pages; generated globe timestamp restored and absent from the diff. All 4 recovered files are byte-identical to the live recorded statement (`20260901022725` bd765a4b…, `20260901024429` da9c1124…, `20260902021703` 1298052b…, `20260902021818` 953591d2…). Drift generator identified: `created_by` attributes 25 of 30 drifted versions to `harbourviewcompany@gmail.com`, i.e. Supabase MCP `apply_migration` — which `SKILL.md` §4 actively recommended while never stating it writes no repository file; fixed in `SKILL.md` and `AGENTS.md`. **Retraction:** `MIGRATION_DRIFT_20260830.md` states the applied SQL is "not recoverable" and that the table stores "`version` and `name` only"; the table in fact has a `statements` ARRAY column holding the SQL verbatim, and that error is why the versions were never reconciled. PR #1740 audited against production: 23/24 faithful, 1 (`20260830185137`) drops 848 bytes of jurisdiction-classification rationale with the SQL byte-identical — reported on that PR, not corrected here to avoid collision. `Promote Production` has failed on every push since 2026-08-30 12:38 UTC while Vercel's git integration deployed `main` regardless, so the gate blocked nothing; left open for sign-off | Branch `claude/updates-repo-review-kyd0vv`; base `9d6d365`; `docs/control/MIGRATION_DRIFT_20260902.md` | Current — nothing applied to production; this change set is repository-only |
