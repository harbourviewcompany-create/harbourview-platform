...
|---|---|---|---|---|---|
| 2026-08-12 | Command Centre section strip clipped mid-row on mobile | `npx vitest run tests/dashboard/mobileCommandSecondaryNav.test.ts`; `npm run lint`; `npm run typecheck`; `npm run test`; `npm run build`; regression suites `mobileCommandCentreV2` + `mobileIntelInstitutional` + `global-css-regression` | PASS — operator-reported: the Overview group's tabs rendered as a half-cut row (`Clinical / Compliance / Education path` sliced) with no scroll affordance. Root cause: `.hvm2-root` is a fixed `100dvh` flex column with `overflow: hidden`, and `.hvm-op-secondary-nav` was `flex: 0 1 auto` + `min-height: 0` + `flex-wrap: wrap` + `max-height: 30dvh` + `overflow-y: auto` — flex-shrink 1 with no min-content floor, so the parent shrank the nav below its content height and `overflow-y` sliced a row. Compounding it, `MobileIntelInstitutional.css` applied *horizontal* affordances (90deg edge-fade mask, `overscroll-behavior-x`, `scroll-padding-inline`) and the reveal logic uses `scrollIntoView({ inline: 'center' })` — three layers assumed a horizontal strip while this one rule wrapped vertically, so the "more content" cue pointed the wrong way. Fixed to a single non-shrinkable row scrolling on the inline axis. New guard proven non-vacuous: 2 of its 4 assertions **fail** against the original CSS and all 4 pass after. lint 0 errors, typecheck exit 0, all suites pass, build exit 0 | Branch `fix/mobile-command-secondary-nav-clipping-20260812`; Overview group carries 10 sections — the largest group and the one reported | **Current — verified branch head** |
