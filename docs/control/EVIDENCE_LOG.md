...
| 2026-07-21 | Globe `signals.country_iso2` TODO — verified premise before touching anything (guardrail in `docs/INTELLIGENCE_ARCHITECTURE_SPEC.md` §9.1) and found the DB-side fix already shipped | Live Supabase check via MCP `execute_sql` on `zvxdgdkukjrrwamdpqrg`: confirmed `signals.country_iso2`/`geo_scope`/`geo_region` already exist (migration `20260716195743_signals_country_iso_resolution`, applied 2026-07-16) and are populated on every insert/update by trigger `trg_signals_resolve_geo` (7,741/8,725 rows resolved; the remaining 649 are all genuine regional/global buckets — 0 unresolved country strings). `npm run typecheck` (0 errors), `npm run lint` (152 problems, all pre-existing on `main`, zero new in changed files — confirmed with a targeted `eslint` run on just the two changed files, 0 output), `npm run test` (57/57 passed), `npm run build` (clean) | The app layer (`lib/globe/supabaseGlobeData.ts`, `components/globe/GlobeProvider.tsx`) was still re-deriving country→ISO2 client-side via a small hand-maintained alias map (`lib/globe/countryAlias.ts`), duplicating and drifting from the already-correct DB column. Repointed both the initial batch load and the realtime `signals` INSERT/UPDATE handler to read `country_iso2` directly off the row; deleted `lib/globe/countryAlias.ts` (dead code once both call sites stopped importing it, confirmed via repo-wide grep). No schema change, no migration — additive/reversible code-only fix. | Branch `claude/todo-implementation-bz8y01` | Current |
