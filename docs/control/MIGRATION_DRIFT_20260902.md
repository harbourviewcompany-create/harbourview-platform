...
Recovery of four production-applied migration versions that had no repository file and were not claimed by open PRs #1739/#1740/#1742/#1743. Files are repository records of already-applied production SQL (byte-identical to `supabase_migrations.schema_migrations.statements`). Nothing in this change set is applied to production.
...
Each was verified with `md5(statements[1])` against the live ledger before write.
...
These four version files are already present on `main` via #1740 reconstruction. This change retains documentation, skill/AGENTS discipline updates, and this control report so the drift-generator lesson and verification trail remain in-tree.
