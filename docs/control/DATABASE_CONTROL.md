...
- Migration 2 — `supabase/migrations/20260801150000_api_expose_quality_and_routing_columns.sql`
  ...
  - Views updated (columns **appended only** — `create or replace view` cannot drop or reorder, which is what killed the 20260715085540 attempt): `public.signals_quality` 30→47, `api.signals_quality` 27→47, `api.signals` 32→47, `api.cc_watch_rules` 8→11. New `api.role_families`, `grant select` to `anon`/`authenticated` only.
