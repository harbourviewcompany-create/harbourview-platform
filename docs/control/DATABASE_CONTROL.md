...
- Migration 1 — `supabase/migrations/20260731120000_signal_role_family_routing.sql`
  ...
  - Adds `signals.role_families text[]`, `signals.routing_version text`, `signals.routed_at timestamptz`; partial GIN index on `role_families`; `(country_iso2, date desc)` index.
...
- Required tests: `npm run test:signal-routing` (35 assertions) covers the read-side matcher. No test covers view shape — that is the gap the CI check above would close.
