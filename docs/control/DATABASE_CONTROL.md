...
- Environment: production (`zvxdgdkukjrrwamdpqrg`)
- Trigger: Tyler approved all three remaining `STAGE3_PROMOTION.md` open items explicitly ("All of them") in response to a direct enumeration of each with its risk profile.
- **Crons enabled:** `cron.alter_job(47, active => true)` / `cron.alter_job(48, active => true)` — `hv-quality-pipeline` (*/2min) and `hv-quality-promote` (*/10min) now run continuously. Migration: `supabase/migrations/20260722021500_enable_hv_quality_pipeline_and_promote_crons.sql`. This is the consequential piece — the live public feed is now written unattended on this schedule going forward, not just via manual invocation.
- **Pipeline A deprecated:** `COMMENT ON TABLE public.signal_classifications` / `COMMENT ON FUNCTION api.promote_classified_signals(numeric,boolean,integer)`, both marked `DEPRECATED 2026-07-22`. No drop, no data touched — scoped deliberately narrower than the migration's own `STAGE3_PROMOTION.md` rollback section (which describes a full drop) at Tyler's stated preference for the lower-risk option. Migration: `supabase/migrations/20260722021600_deprecate_unused_stage3_pipeline_a.sql`.
- **`api.rows_needing_titles` fixed:** discovered while scoping the title backfill — the RPC joined against `public.signal_classifications` (Pipeline A, just deprecated above), so it could only ever reach rows classified by the deprecated path. Live-verified before the fix: of 919 promoted rows missing `editorial_title`, only 9 were reachable. Fixed to match `signals.quality_label='signal'` directly. Migration: `supabase/migrations/20260722021700_fix_rows_needing_titles_pipeline_b.sql`. Same authorization check preserved unchanged.
- **Title backfill itself: NOT run.** Scoping query (post-fix) found 904 live-promoted rows missing a title (close to the originally-cited 919, small drift from elapsed time) plus a separately-discovered 3,519-row unpromoted backlog also missing titles — the backlog was not part of what was approved and was deliberately not touched. Every attempt to invoke `hv-classify` in `mode=titles` (the only path that actually calls the paid LLM providers and writes `editorial_title`) was blocked by the Claude Code Auto Mode classifier — first via a `curl` loop in Bash, then a single isolated `curl` retry, both denied consistently (unlike the transient blocks on the grant-revoke migration earlier in the session, which succeeded on retry with identical SQL). No MCP tool exists to invoke a Supabase Edge Function directly (`deploy_edge_function`/`get_edge_function`/`list_edge_functions` only). Deliberately did not attempt `net.http_post` from SQL as a substitute path — same paid-API-spend action through a different door, not a genuinely different action. Left for Tyler to run directly or explicitly permission. Exact command and cost estimate recorded in `docs/control/STAGE3_PROMOTION.md`'s Owner decisions section.
- RLS: unaffected by any of the three changes — no policy touched.
- Public API routes affected: none for the deprecation/RPC-fix (internal functions only). The cron changes affect what appears in the public Intel feed (`signals?reviewed=eq.true`) going forward, on an ongoing basis rather than a single point-in-time change.
- Backward compatibility: the deprecation comments and `rows_needing_titles` fix are additive/restrictive only (`rows_needing_titles`' signature/return shape unchanged, only its `WHERE` predicate changed — no caller-visible behavior change beyond "now actually finds the right rows"). **The cron activation is not a backward-compatible/additive change in the same sense — it is a behavioral production change**: the pipeline now runs and writes to the live public feed continuously and unattended going forward, where before it required a manual call. Requires ongoing monitoring (`cron.job_run_details` for jobids 47/48) and rollback awareness (see below), not just a one-time compatibility check.
- Rollback: crons — `select cron.alter_job(47, active => false); select cron.alter_job(48, active => false);`. Deprecation comments — `comment on table ... is null;` / `comment on function ... is null;` to clear (table/function still exist, comment-only). `rows_needing_titles` — restore the `signal_classifications` join (see 2026-07-21 migration history) — not recommended, restores the near-total miss rate.
- Required tests: none applicable (no test suite covers these RPCs/cron jobs). Verified live: `cron.job.active = true` for jobids 47/48; `obj_description()` confirms both deprecation comments; the `rows_needing_titles` fix verified by re-running its `WHERE`-clause logic directly as a `SELECT COUNT` before deciding on backfill scope (904 reachable vs. 9 before the fix).
- Human approval status: Tyler approved all three ("All of them") after being shown the specific risk framing for each. The title-backfill's own execution remains blocked pending either Tyler running it directly or granting the specific tool permission — not a scope disagreement, a tooling limitation discovered mid-task.

## 2026-07-22 (part 4) — Incident: quality-pipeline crons failing on nearly every run; harvest fixed, promote left disabled

- Environment: production (`zvxdgdkukjrrwamdpqrg`)
- Trigger: follow-up "is anything materially important missing" check found both crons enabled above were failing on nearly every run, not caught at enablement time.
- Tables/functions affected: no schema change. `hv_classify_jobs`/`net._http_response` statistics refreshed (`ANALYZE`, not DDL). No function bodies changed this round — the fix was purely statistical.
- Root cause 1 (fixed): stale planner statistics on `hv_classify_jobs` (never auto-analyzed, 89,013 rows) caused a Nested Loop plan that re-scanned `net._http_response` once per outer row instead of a single Hash Join, timing out `hv_classify_corpus_harvest()`. Fixed via `ANALYZE`; verified plan change and a clean manual harvest call (120 rows, no timeout). Migration: `supabase/migrations/20260722030000_analyze_hv_classify_jobs_fix_harvest_timeout.sql`.
- Root cause 2 (not fixed, deliberately left disabled): `hv_dedup_assign()` is a real O(n²) self-join over embedded signals (5,080 rows in scope, ~25.8M comparisons), not expressible as an indexable KNN query — `ANALYZE` cannot fix this shape of query, and it worsens as the corpus grows. Needs a design fix (batching/narrower scope/KNN rewrite), not attempted this session.
- RLS: unaffected. Public API routes: unaffected directly — `hv-quality-pipeline` resumed writing to `signals.quality_label`/etc. (not the public-facing `reviewed` flag); `hv-quality-promote` (which does flip `reviewed`) stays off.
- Backward compatibility: `ANALYZE` is a read/statistics-only operation — zero data or behavior risk by construction.
- Rollback: none needed for the `ANALYZE` migration. To undo the re-enablement: `select cron.alter_job((select jobid from cron.job where jobname='hv-quality-pipeline'), active => false);`.
- Required tests: none applicable. Verified live: `EXPLAIN` plan before/after `ANALYZE` (Nested Loop → Hash Join); manual `hv_classify_corpus_harvest()` call succeeded; `cron.job.active` confirms final state (jobid 47 true, jobid 48 false); cost-exposure check via `hv_classify_jobs`/`net._http_response` join confirmed only one dispatch batch went out all session (not a runaway-spend incident, corrected an overstated claim made earlier in the same conversation before verifying).
- Human approval status: Tyler approved the disable→diagnose→fix→validate→re-enable sequence explicitly ("Go") after it was laid out in detail. The dedup fix itself was not proposed or attempted — flagged as a distinct follow-up needing its own scoping.

## 2026-07-22 (part 5) — Security: revoke PUBLIC EXECUTE on 12 public.hv_* pipeline functions

- Environment: production (`zvxdgdkukjrrwamdpqrg`)
- Trigger: CodeRabbit review comment flagging `public.hv_promote_signals` missing a `PUBLIC EXECUTE` revoke; live grant check found the same gap on the full Pipeline B function family, not just the one function flagged.
- Functions affected (grant only, no body change): `hv_promote_signals`, `hv_dedup_assign`, `hv_pipeline_tick`, `hv_quality_promote_tick`, `hv_classify_corpus_dispatch`, `hv_classify_corpus_harvest`, `hv_translate_dispatch`, `hv_translate_harvest`, `hv_embed_dispatch`, `hv_embed_harvest`, `hv_entities_dispatch`, `hv_entities_harvest` — all `public` schema, all SECURITY DEFINER, none with any internal authorization check (unlike the `api.*` RPCs hardened 2026-07-21/22).
- Bug: all 12 were reachable via the `PUBLIC` grant with zero internal check. `hv_classify_corpus_dispatch` accepts arbitrary caller-supplied `p_limit`/`p_scope_days` and dispatches paid LLM calls — an anon caller could invoke it repeatedly to run up cost with no rate limit. The tick functions and `hv_promote_signals`/`hv_dedup_assign` could be triggered on demand, bypassing cron scheduling.
- RLS: unaffected — grant-only change, no policy touched.
- Public API routes affected: all 12 functions, if exposed via PostgREST at all, are no longer callable by `anon`/`authenticated`. `postgres`/`service_role` grants untouched.
- Backward compatibility: additive/restrictive only. Verified via `cron.job`: both jobs that invoke this pipeline (`hv-quality-pipeline`, `hv-quality-promote`) run as `username='postgres'`, which retains its own explicit grant — nothing that actually runs today is affected.
- Migration file: `supabase/migrations/20260722031500_revoke_anon_authenticated_hv_pipeline_functions.sql` — see that file's header for a recorded self-correction: the first applied version revoked from `anon, authenticated` explicitly, which didn't work because both roles still inherited access via the untouched `PUBLIC` grant. Caught via an immediate post-apply grant re-check, corrected to `REVOKE ... FROM PUBLIC`, re-verified.
- Rollback: `grant execute on function <fn> to public;` per function — not recommended, restores the unauthenticated exposure.
- Required tests: none applicable (no test suite covers these functions). Verified live: `pg_proc.proacl` re-queried for all 12 post-fix, confirms only `postgres`/`service_role` remain; `cron.job.username` confirms both cron jobs run as `postgres`.
- Human approval status: Tyler approved explicitly ("Confirming") after the finding, severity, and fix were laid out in detail, per the security/auth-change confirmation rule.

## 2026-08-01 — Signal→operator routing columns + api view drift (FOURTH occurrence of the stale-view bug class)

- Environment: production (`zvxdgdkukjrrwamdpqrg`). **Neither migration has been applied.** Both are authored and committed only; applying needs Tyler's sign-off (CLAUDE.md Rule 3c, spec §9 guardrails #1/#7/#8).
- Trigger: PR #1231 review. Codex flagged that new columns would not be visible through the `api` schema. Verifying that surfaced a larger, pre-existing, live defect.
- Migration 1 — `supabase/migrations/20260731120000_signal_role_family_routing.sql`
  - Adds `public.role_families` (15-row reference table, RLS on, `select` to `anon`/`authenticated`, no write policy → service-role only).
  - Adds `signals.role_families text[]`, `signals.routing_version text`, `signals.routed_at timestamptz`; partial GIN index on `role_families`. **No `(country_iso2, date desc)` index is added** — `idx_signals_iso_date` from `20260716195743_signals_country_iso_resolution.sql` is already byte-for-byte identical (confirmed live), so adding one would maintain two identical B-trees for no query benefit. Recorded here and in the migration header so it is not re-added.
  - Adds `cc_watch_rules.country_iso2 text[]`, `role_families text[]`, `min_impact text` + CHECK.
  - Adds `public.hv_assert_routable_role_families()` (SECURITY DEFINER, `search_path` pinned, `EXECUTE` revoked from PUBLIC) and BEFORE INSERT/UPDATE triggers on both `role_families` columns. Without it the reference table enforced nothing — any invented key, or the internal `harbourview_admin_operator`, was accepted by Postgres. NULL/empty arrays stay valid ("no filter" / "not yet routed").
  - Explicit `begin;`/`commit;` removed — the Supabase CLI already wraps each file, and only 14 of 764 migrations here nest manually.
  - `CREATE INDEX CONCURRENTLY` deliberately **not** used (SQLFluff PG01 / Squawk): it cannot run in a transaction, and `public.signals` is ~12.5k rows where a plain build is milliseconds. Reasoning recorded in the file header so it is not relitigated.
- Migration 2 — `supabase/migrations/20260801150000_api_expose_quality_and_routing_columns.sql`
  - **Pre-existing live defect, introduced by PR #1214 (2026-07-30), found here:** `lib/dashboard/dashboardServerData.ts` selects `analysis` + `SIGNAL_QUALITY_SELECT` from `signals_quality` and orders by `quality_confidence`; `api.signals_quality` carries none of those columns. Confirmed live: `select quality_confidence from api.signals_quality` → `42703 column does not exist`. PostgREST 400s and the caller's `if (!error && data...)` falls through to `listIaSignals()` — `ia_signals`, **641 rows**, against the 12,465-row classified corpus. The Command Centre has been serving the wrong, much smaller source since #1214 merged, invisibly.
  - Root cause of the split: `pgrst.db_schemas = "public, graphql_public, job_search, api"` with `public` first, so raw `fetch('/rest/v1/...')` resolves to `public` (works) while supabase-js clients pinned to `db.schema='api'` resolve to `api` (broken). `lib/supabase/env.ts`'s comment claiming `public` is *not* exposed is stale and should be corrected separately.
  - Views updated (columns **appended only** — `create or replace view` cannot drop or reorder, which is what killed the 20260715085540 attempt): `public.signals_quality` 30→47, `api.signals_quality` 27→47, `api.signals` 32→48, `api.cc_watch_rules` 8→11. Prefix preservation verified mechanically against the live `information_schema` ordering — see the 2026-08-01 column-order validation row in `EVIDENCE_LOG.md`. New `api.role_families`, `grant select` to `anon`/`authenticated` only.
- RLS posture: unchanged on all base tables. New `role_families` table is RLS-on, read-only to browser roles. No grant widened anywhere; no policy altered.
- Public/private boundary: unaffected. No counterparty field exposed; HAR-99/101 untouched. Columns added are classifier output and a role-family vocabulary — internal/public reference data, no PII.
- Backward compatibility: additive throughout. No base table altered beyond nullable column adds (metadata-only in modern Postgres), no row mutated, no view filter changed.
- **Deliberately NOT fixed — needs an explicit decision.** `public.signals_quality`'s WHERE clause still selects rows using `score`: `reviewed = true OR cat NOT IN ('SOURCE_ENGINE','GAZETTE') OR (cat='SOURCE_ENGINE' AND score BETWEEN 50 AND 89) OR (cat='GAZETTE' AND score >= 70)`. `score` is the legacy keyword scorer documented as **inverted** in `INTELLIGENCE_ARCHITECTURE_SPEC.md` §2.5 and retired as a promotion instrument — so a view named `signals_quality` picks its rows with the one number the platform agreed not to trust. Changing which rows the Command Centre shows is a product decision, not a drift fix; this migration only makes the columns readable.
- Standing recommendation: this is the **fourth** occurrence of "base column added, exposed view never refreshed" (2026-07-13/14, 2026-07-15, 2026-07-15 part 2, now). The pattern will keep recurring. A CI check comparing each `api.*` view's column list against its base table would end the class outright.
...
- **Migrations (required order):** `20260808190000_decision_intel_stage0_first_slice.sql`, corrective `20260808203000_decision_intel_stage0_review_fixes.sql`, then completion hardening `20260810202000_decision_intel_stage0_completion_hardening.sql`. All three are required for the Stage 0 first-slice deployment contract.
- **Canonical ownership:** existing `source_registry`/`source_snapshots`, `hv_evidence`, and `public.jurisdictions` remain authoritative; `public.jurisdiction_crossref` is the ISO-2 → canonical jurisdiction identity bridge; Stage 0 adds only evidence references, assertions, events, assessments/versions, recommendations and allowlisted dossier/route projections.
- **RLS/exposure:** raw Stage 0 objects remain staff-controlled. Product-tier customer reads do **not** receive direct canonical-table or view grants. Authenticated Intel/operator dossier access is exposed through the tier-gated `SECURITY DEFINER` RPC `api.get_intel_event_dossier(text)` after an explicit event-level customer publication classification; verification alone never authorizes exposure. `api.resolve_intel_dashboard_routes(text[])` is service-role-only and returns route identity, current customer displayability, and the canonical recommendation posture for server-side dashboard hydration. Anonymous execution is revoked. Raw `hv_evidence`, storage paths, private notes, canonical review metadata and Marketplace-private data remain outside the product projection.
- **Review hardening:** one snapshot can support multiple signal refs; legacy signal IDs remain durable lineage keys; confidence is constrained to 0–1; new events default to `needs_review`; assessment versions are append-only and migration-created version 1 is repaired to the complete canonical field set; rejected/superseded assertions and recommendations are excluded; contradiction relationships are projected; clustered signals resolve to one event; `signals.country_iso2` resolves through `jurisdiction_crossref`; upstream withdrawal/rejection/unreview/reclassification suppresses the linked canonical chain and customer visibility while preserving canonical route ownership; desktop/mobile dossier-link eligibility is derived from the actual customer-display projection while retaining legitimate unowned legacy/IA compatibility.
...
- **Rollback/forward-fix:** production remains unapplied. Apply all three migrations in the required order only after disposable verification; use reviewed forward migrations for post-apply defects.
- **Required tests:** production-shaped three-migration apply/backfill/RLS, canonical jurisdiction mapping, zero auto-verified rows, lineage/cardinality, explicit customer-publication classification, upstream-withdrawal suppression, complete assessment version-1 payload, canonical dashboard displayability routing, five-width dossier Playwright, authenticated nine-width Command Centre visual evidence, leakage/security and the full exact-head PR workflow matrix.
