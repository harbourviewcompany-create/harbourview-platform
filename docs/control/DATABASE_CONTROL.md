...
- Backward compatibility: all three changes additive/restrictive only. `rows_needing_titles` signature/return shape unchanged, only its `WHERE` predicate changed — no caller-visible behavior change beyond "now actually finds the right rows."
