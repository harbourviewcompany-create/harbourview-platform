...
The customer/product execution boundary is the allowlisted RPC layer in the exposed `api` schema:

- `api.get_intel_event_dossier(text)` — returns the customer-safe dossier projection for one canonical event after product entitlement and publication classification checks.
- `api.resolve_intel_event_route(text)` — resolves a displayable upstream signal identifier to its canonical event identifier for authenticated product navigation.
- `api.resolve_intel_dashboard_routes(text[])` — service-role-only server helper returning canonical ownership plus current customer displayability for dashboard records; it is not executable by `anon` or `authenticated`.

The two customer RPCs perform their own authenticated subscription-entitlement checks and permit only `intel` or `operator` tier access. Anonymous callers and free-tier callers are rejected. The functions execute with fixed safe `search_path` values and expose only their declared return columns.

The completion migration adds an explicit `intel_events.customer_visibility` publication classification. Verification and publication are independent: a canonical event may be `verified` while remaining `customer_visibility='internal'`, in which case it is absent from the customer dossier projection. The customer dossier RPC reads only `public.intel_customer_event_dossiers`, which combines the existing allowlisted dossier shape with that explicit publication classification.

The earlier security-invoker-view design is superseded by this RPC boundary. Product subscribers are not granted direct SELECT on the canonical base tables, `public.intel_event_dossiers`, `public.intel_customer_event_dossiers`, or `public.intel_event_route_map`; those relations are implementation details behind the RPCs. This prevents customer queries from bypassing the allowlisted dossier projection to obtain lineage IDs, review metadata, or other staff-operational columns.

Raw evidence bodies, storage paths, private notes, Marketplace-private data, and service credentials are not projected by any product RPC.
...
The completion migration makes upstream surfaceability authoritative for migrated lineage. If an upstream `public.signals` row is deleted, rejected, unreviewed, or reclassified into an excluded classifier/content state, the linked canonical assertion/event/assessment/recommendation chain is moved back to `needs_review`, the event is classified `customer_visibility='internal'`, and canonical route ownership is retained so legacy fallback cannot resurrect the withdrawn intelligence.

## Migration safety

The implementation uses three ordered additive migrations:
...
All three are required for the Stage 0 first-slice deployment contract. Applying only the first two omits the explicit customer-publication boundary, upstream-withdrawal propagation, server dashboard displayability resolver, and complete migration-created assessment version-1 repair.

They do not mutate or delete `signals`, `ia_signals`, source acquisition tables, Marketplace tables, watchlists, Actions, Clinical data or public `/signals` projections. The completion migration adds suppression triggers on `public.signals`; those triggers update only the linked Stage 0 canonical chain when upstream surfaceability is withdrawn and do not modify the upstream signal row themselves.
...
1. All three SQL migrations parse and apply in order against a production-shaped database.
...
8. Assessment versions reject UPDATE and DELETE, and migration-created version 1 contains the exact complete canonical assessment field set used by subsequent versions.
...
11. Anonymous and free-tier access to customer product RPCs fails; `api.resolve_intel_dashboard_routes(text[])` is service-role-only.
...
15. Verified-but-internal events are absent from customer dossier reads until an explicit customer publication classification is set.
16. Withdrawing/rejecting/unreviewing/reclassifying an upstream signal suppresses the linked canonical chain while preserving canonical ownership.
17. Dashboard dossier-link eligibility is derived from the actual customer-display projection so suppressed canonical events do not advertise 404 links; unowned legacy/IA compatibility remains available.
18. Existing `/signals`, Marketplace, Clinical and Actions regression checks stay green.
19. The authenticated dossier passes Playwright at 320×700, 375×812, 390×844, 430×932 and desktop.
20. The authenticated Command Centre visual gate passes all nine configured responsive widths on the exact PR head.
