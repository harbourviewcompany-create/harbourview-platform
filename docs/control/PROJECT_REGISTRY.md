...
| Canonical Vercel deployment | Unresolved | HOLD | Accessible Vercel team `harbourviewnetwork` exposes only `chatbot`. GitHub status context `Vercel – harbourview` points to inaccessible workspace/project path `harbourviewcannabis-3379s-projects/harbourview`. `https://harbourview.vercel.app` and `https://harbourview-platform.vercel.app` remain unconfirmed. |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| GitHub / App | Harbourview Platform | Provisional canonical production app | Main Harbourview marketplace, intelligence, admin, intake and public platform surface | `harbourviewcompany-create/harbourview-platform` | Unverified. GitHub status context `Vercel – harbourview` exists, but direct Vercel inventory access only shows unrelated `chatbot` project. | `zvxdgdkukjrrwamdpqrg` likely target | Intended/claimed target: `https://harbourview.vercel.app`; not proven through accessible Vercel inventory. `https://harbourview-platform.vercel.app` also unproven. | Public repo | Open/stale PR debt, deployment-target ambiguity, Cloudflare/Vercel command sprawl, public/private boundary risk | Keep and stabilize as canonical unless formally replaced | Tyler / Harbourview | Resolve Vercel linkage, close stale PRs, run canonical verification gates | Confirm this remains canonical production app |
...
| Vercel / Deployment | Harbourview Vercel Target | Missing from accessible inventory | Intended Harbourview production/preview deployment target | Expected repo: `harbourview-platform` or future `harbourview-network` | GitHub status context `Vercel – harbourview` points to `harbourviewcannabis-3379s-projects/harbourview`, but that workspace/project is not accessible through the connected Vercel tool. | Expected DB: `zvxdgdkukjrrwamdpqrg` | `https://harbourview.vercel.app` remains intended/claimed but unconfirmed; `https://harbourview-platform.vercel.app` also unconfirmed. | Unknown | Highest infrastructure ambiguity; cannot trust deployment-control state until resolved | Resolve before more launch work | Tyler / Harbourview | Get access to the Vercel workspace/project behind `Vercel – harbourview`; record project ID, linked repo, branch, domains and Git auto-deploy setting | Identify actual canonical Vercel project |
| Vercel / Deployment | Chatbot Vercel Project | Active separate deployment | Production deployment for chatbot app | `harbourviewcompany-create/chatbot` | `chatbot` | None found | `chatbot-harbourviewnetwork.vercel.app` | Visible in Vercel team `harbourviewnetwork` | Only visible Vercel project; may mask missing Harbourview platform workspace/project scope | Keep separate | Tyler / Harbourview | Document env, domains and whether it should be under Harbourview product umbrella | Decide naming/domain strategy |

## External Deployment Context Classification

| Context | Classification | Current disposition |
|---|---|---|
| `Vercel – harbourview` | Active GitHub status context pointing to inaccessible Vercel workspace/project path `harbourviewcannabis-3379s-projects/harbourview` | HOLD until workspace/project ID, linked repo, production branch, domains and Git auto-deploy setting are confirmed |
| `Vercel – harbourview-platform-rod3` | Duplicate/stale candidate observed as a separate Vercel context | Remove from required checks or disconnect if not canonical after Vercel inventory is complete |
| `netlify/harbourview-platform/deploy-preview` | Duplicate/stale candidate | Remove from required checks or disconnect if not canonical |
| `netlify/harbourviewns/deploy-preview` | Duplicate/stale candidate | Remove from required checks or disconnect if not canonical |
| `netlify/harbourview-international/deploy-preview` | Duplicate/stale candidate | Remove from required checks or disconnect if not canonical |
| Vercel `chatbot` | Separate visible Vercel project under team `harbourviewnetwork` | Keep outside Harbourview Platform deployment control unless explicitly integrated |
...
6. External deployment/status contexts must not be treated as canonical proof unless the registry identifies the backing project, workspace/team, linked repo, branch and domains.
...
- Harbourview Vercel production target is not visible in the accessible Vercel inventory.
- Connected Vercel team `harbourviewnetwork` exposes only the separate `chatbot` project.
- GitHub status context `Vercel – harbourview` points to inaccessible workspace/project path `harbourviewcannabis-3379s-projects/harbourview`.
- `https://harbourview.vercel.app` and `https://harbourview-platform.vercel.app` remain unconfirmed through accessible Vercel tooling.
