...
- **Deploy-integration sprawl outlived the Jul 1 consolidation.** Five git integrations still fire on every PR against a repo whose canonical target is Vercel: Vercel (✅), Cloudflare **Pages** on account `c9bde393…` (✅), Cloudflare **Workers Builds** on account `4a7c450c…` (❌), Cloudflare **Workers Builds** on account `c9bde393…` (❌), and Netlify `harbourviewns` (✅, still building previews despite `netlify.toml` having been removed).

  **Two separate Cloudflare accounts each run a Workers Builds integration, and both emit a check run with the byte-identical name `Workers Builds: harbourview-platform`.** Observed across two commits on this PR: on `2e7db7c` the `4a7c450c…` build failed while `c9bde393…` skipped; on `ef08b6b` the `c9bde393…` build failed (check run `90754801856`) while `4a7c450c…` was still running. Because the names collide, GitHub's check-runs listing surfaces only the most recent, so **which account is failing is not visible from the PR checks UI** — it can only be read from the `details_url` account ID. This is very likely why the failure has appeared intractable: fixing or disconnecting one account leaves the other still failing under the same check name.

  Correct remediation is to disconnect the Workers Builds git integration on **both** accounts. Cloudflare **Pages** on `c9bde393…` is a *separate* integration on that same account and is passing — leave it connected. All of this is dashboard-side work only Tyler can do.
