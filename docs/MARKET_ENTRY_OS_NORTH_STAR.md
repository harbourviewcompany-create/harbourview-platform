...
**Version 1.6** · 2026-07-18 · supersedes v1.5
...
All four of the original blocking decisions are resolved (as of v1.5, 2026-07-18): **business model is per-report** (Tyler — one-time fee per corridor plan generated, not subscription or take-rate), **`jurisdiction_playbooks` is the canonical knowledge graph**, **CounterpartyStub disclosure holds at HAR-99/101** (no raw counterparty contact data in report output — see Bigger Picture table), and **network integration is reports-only, no automated intro-triggering** (see same table). No new schema work is blocked by an open decision as of this version.

As of v1.6, one execution gap surfaced by the per-report decision is also addressed: the **payment mechanism** is decided (a new one-time Stripe Checkout path, `mode: 'payment'`, separate from the existing subscription system — see Bigger Picture table) but deliberately not yet implemented, since no report/corridor-plan data model exists to attach a purchase to. Recommended path: build one corridor (Canada → Germany) end-to-end starting with the Documentation Engine and Workflow Engine — now the actual product being sold, not a bonus on top of a subscription — wiring the payment path into the same PR that creates the report data model, before generalizing.
...
| Payment mechanism for per-report | ⚠️ Mechanism decided 2026-07-18, implementation deliberately deferred | Checked the existing Stripe integration (`lib/stripe/server.ts`, `lib/billing/*`, `app/api/stripe/checkout/route.ts`): it's subscription-only — `mode: 'subscription'`, tier-gated via `entitlements.ts`'s `FEATURE_TIER_MAP` (free/starter/professional/enterprise) for the existing platform (signals, intelligence, network, etc.). That's the wrong shape for a one-time per-report purchase. **Decision:** a second, parallel Stripe Checkout path using `mode: 'payment'` (one-time), reusing the existing `stripe` client and `getOrCreateStripeCustomer` helper — not a new tier, not a change to the existing subscription system, which stays exactly as-is for the rest of the platform. **Deliberately not implemented yet:** a repo-wide search found no `corridor_reports`/`corridor_plan`/`mission_report` table or equivalent — there is nothing yet to record a purchase against or attach a generated document to. Writing the checkout route now would mean guessing at a schema instead of deriving it from the real Documentation Engine (6) build. Build trigger: when Layer 6's data model lands, add the purchase reference there and wire this checkout path in the same PR, not as separate, disconnected work. |
...
- **v1.6** (2026-07-18): Investigated the payment mechanism for per-report, at Tyler's direction ("execute 2" — the payment-mechanism gap flagged when asked what's still materially missing). Checked the existing Stripe integration (`lib/stripe/server.ts`, `lib/billing/entitlements.ts`, `app/api/stripe/checkout/route.ts`): subscription-only, tier-gated, wrong shape for a one-time per-report purchase. Decided a second, parallel Stripe Checkout path (`mode: 'payment'`) reusing the existing client — not a new tier, not a change to the existing subscription system. Deliberately did not implement it: no `corridor_reports`/`corridor_plan` table or equivalent exists yet to attach a purchase to, and a repo-wide search confirmed it. Building the checkout route now would mean guessing at a schema instead of deriving it from the real Documentation Engine (6) build — flagged as the concrete trigger for when to build this instead.
